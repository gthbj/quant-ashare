...
DECLARE market_state_version STRING DEFAULT 'market_state_v0_20260606';
...
OPTIONS (
  description = 'Daily market state and risk-off trigger features for Strategy 1 tail-risk P2; signals are formed after t close and usable from t+1 open',
...
index_base AS (
  ...
  WHERE idx.trade_date BETWEEN lookback_start_date AND dws_end_date
    AND idx.sec_code IN ('000300.SH', '000852.SH')
...
ALTER COLUMN market_state_version SET OPTIONS (description = 'Market-state rule version'),
...
ALTER COLUMN is_risk_off SET OPTIONS (description = 'TRUE when v0 market-risk triggers enter risk-off'),
...
ALTER COLUMN risk_off_action SET OPTIONS (description = 'P2 action; v0 only supports skip_new_buys');

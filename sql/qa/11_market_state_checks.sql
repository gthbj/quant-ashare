...
DECLARE p_legacy_market_state_version STRING DEFAULT 'market_state_v0_20260606';
...
ASSERT (
  ...
  WHERE ms.trade_date BETWEEN DATE_ADD(p_start_date, INTERVAL 80 DAY) AND p_end_date
    ...
    AND (
      ...
      OR ms.sse_composite_ret_20d IS NULL
      OR ms.sse_composite_drawdown_20d IS NULL
      OR ms.sse_composite_vol_20d IS NULL
    )
) AS 'QA-MKT-7: index market-state metrics including SSE Composite must be populated after warm-up';

ASSERT (
  p_market_state_version = p_legacy_market_state_version
  OR (
    SELECT COUNT(*) = 0
    FROM (
      SELECT
        ms.trade_date,
        COUNTIF(ms.market_state_version = p_legacy_market_state_version) AS legacy_rows,
        COUNTIF(ms.market_state_version = p_market_state_version) AS current_rows
      FROM `data-aquarium.ashare_dws.dws_market_state_daily` AS ms
      WHERE ms.trade_date BETWEEN p_start_date AND p_end_date
        AND ms.market_state_version IN (p_legacy_market_state_version, p_market_state_version)
      GROUP BY ms.trade_date
      HAVING legacy_rows != 1 OR current_rows != 1
    )
  )
) AS 'QA-MKT-8: legacy v0 and current v1 market-state versions must both be present exactly once per trade date';

ASSERT (
  SELECT COUNT(*) = 0
  FROM `data-aquarium.ashare_dws.dws_market_state_daily` AS ms
  WHERE ms.trade_date BETWEEN DATE_ADD(p_start_date, INTERVAL 80 DAY) AND p_end_date
    AND ms.market_state_version = p_legacy_market_state_version
    AND (
      ms.sse_composite_ret_5d IS NOT NULL
      OR ms.sse_composite_ret_20d IS NOT NULL
      OR ms.sse_composite_drawdown_20d IS NOT NULL
      OR ms.sse_composite_vol_20d IS NOT NULL
      OR ms.sse_composite_close_to_ma20 IS NOT NULL
      OR ms.sse_composite_close_to_ma60 IS NOT NULL
      OR ms.sse_composite_ma20_to_ma60 IS NOT NULL
    )
) AS 'QA-MKT-9: legacy v0 rows must not populate SSE Composite fields; use v1 for those fields';

SELECT
  'QA-MKT completed' AS status,
  p_market_state_version AS checked_market_state_version,
  COUNT(*) AS row_count,

...
ASSERT (
  ...
  FROM `data-aquarium.ashare_dws.dws_stock_feature_price_daily`
  WHERE trade_date BETWEEN dws_start_date AND dws_end_date
    AND history_obs_60d < 61
    AND COALESCE(has_full_history_60d, TRUE)
) AS 'price feature rows with fewer than 61 observations must not be marked full-history';
...
ASSERT (
  ...
  WHERE trade_date BETWEEN dws_start_date AND dws_end_date
    AND history_obs_60d >= 61
    AND NOT COALESCE(has_full_history_60d, FALSE)
) AS 'price feature rows with at least 61 observations must be marked full-history';
...
ASSERT (
  ...
  WHERE trade_date BETWEEN dws_start_date AND dws_end_date
    AND sample_trainable_default
    AND NOT COALESCE(has_full_history_60d, FALSE)
) AS 'default trainable samples must require complete 60d price history';

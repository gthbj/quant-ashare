...
-- 资产负债表最新版本便捷表（默认合并口径）：每个 (sec_code, report_period) 取默认口径最新公告版本。
-- 仅保留 is_default_report_caliber=TRUE（report_type=1）；(sec_code, report_period) 唯一；不用于 PIT 回测。

CREATE OR REPLACE TABLE `data-aquarium.ashare_dwd.dwd_fin_balancesheet_latest`
CLUSTER BY sec_code, report_period
OPTIONS (
  description = '资产负债表默认合并口径最新便捷表，派生自 dwd_fin_balancesheet；仅含 report_type=1，(sec_code, report_period) 唯一；不用于 PIT 回测 join。'
) AS
SELECT *

...
-- 股票日频估值 DWD：写入 2019+ daily_basic 原始日频指标。

DECLARE dwd_start_date DATE DEFAULT DATE '2019-01-01';
DECLARE dwd_end_date DATE DEFAULT CURRENT_DATE('Asia/Shanghai');

CREATE OR REPLACE TABLE `data-aquarium.ashare_dwd.dwd_stock_eod_valuation`
PARTITION BY DATE_TRUNC(trade_date, MONTH)
CLUSTER BY sec_code
OPTIONS (
  description = 'Daily stock valuation and share structure DWD from Tushare daily_basic; rows written from 2019-01-01',
...
ALTER COLUMN turnover_rate SET OPTIONS (description = '换手率'),
ALTER COLUMN pe_ttm SET OPTIONS (description = '滚动市盈率'),
ALTER COLUMN total_mv_cny SET OPTIONS (description = '总市值，元，由 Tushare 万元单位换算');

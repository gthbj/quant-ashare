...
-- 利润表最新版本便捷表（默认合并口径）：每个 (sec_code, report_period) 取默认口径的最新公告版本。
-- 仅保留 is_default_report_caliber=TRUE（report_type=1，合并报表）；(sec_code, report_period) 唯一。
-- 回测/PIT 仍应使用 dwd_fin_income 版本事实表做 as-of join，本表不用于回测，避免后期修正泄漏。

CREATE OR REPLACE TABLE `data-aquarium.ashare_dwd.dwd_fin_income_latest`
CLUSTER BY sec_code, report_period
OPTIONS (
  description = '利润表默认合并口径最新便捷表，派生自 dwd_fin_income；仅含 report_type=1，(sec_code, report_period) 唯一；不用于 PIT 回测 join。'
) AS
SELECT *

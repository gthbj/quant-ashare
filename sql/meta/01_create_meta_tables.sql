...
OPTIONS(
  description = '流水线运行记录：每次 Composer DAG 或 Workflows execution 写入一行'
);
...
ALTER TABLE `data-aquarium.ashare_meta.pipeline_run`
ALTER COLUMN pipeline_run_id SET OPTIONS (description='Pipeline run id（Composer DAG run id 或 Workflows 侧自定义 run id）'),
ALTER COLUMN dag_id SET OPTIONS (description='逻辑流水线名称（可由 Composer DAG 或 Workflow 承载）'),
...
OPTIONS(
  description = '流水线任务状态：每个 Composer task 或 Workflows step 写入一行'
);
...
ALTER TABLE `data-aquarium.ashare_meta.pipeline_task_status`
ALTER COLUMN pipeline_run_id SET OPTIONS (description='Pipeline run id（Composer DAG run id 或 Workflows 侧自定义 run id）'),
ALTER COLUMN task_id SET OPTIONS (description='Airflow task id / Workflow step id'),
...
ALTER COLUMN airflow_log_url SET OPTIONS (description='Airflow task log URL；Workflows 迁移后写 workflow execution/step 控制台 URL'),

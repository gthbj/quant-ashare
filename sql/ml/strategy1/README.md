...
默认配置见 `configs/strategy1/cloudrun_runner_default.yml`，运行手册见 `docs/策略1CloudRun训练回测运行手册.md`。默认配置不再显式写出 `candidate_grid`；`config.py` 会展开 sklearn 原生 logistic parity 网格：7 个 L2、7 个 L1、21 个 elasticnet，共 35 个候选，并在 registry 记录 `candidate_grid_version='sklearn_logistic_parity_v1'` 和 `candidate_grid_size`。
...
sklearn parity 诊断 artifact：selected model artifact 目录额外输出 `candidate_metrics.csv/json`、`selected_coefficients.csv`、`bqml_weight_alignment.csv` 和 `implementation_audit.json`。这些文件用于判断 parity 失败来自候选网格不足、预处理 / class_weight 口径差异、特征顺序 / 缺失率问题，还是 sklearn 与 BQML 求解器差异。`16_qa_cloudrun_runner_outputs.sql` 的 `QA-CR-8` 会校验 selected registry 记录了这些 GCS URI。

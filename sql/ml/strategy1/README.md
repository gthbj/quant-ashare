...
# 21: 风险特征入模搜索 QA（PRD-20260606-03；在 19 QA 后执行）
bq query --use_legacy_sql=false --location=asia-east2 < sql/ml/strategy1/21_qa_risk_feature_search_outputs.sql

# 20: 尾部风险诊断 / P1 个股风险过滤 QA（PRD-20260606-01 路径执行）
...
# 24: 验收门 v3 只读 replay 与源数据一致性 QA（不改 historical accepted/rejected）
# replay_acceptance_gate_v3.py 只读 ADS/DWD，按 v3 contract 对五次正式搜索 Top-K 重算 Sharpe /
# Calmar / 五指数相对门 / final_holdout trading days，并输出独立 replay artifact。
python scripts/strategy1/replay_acceptance_gate_v3.py \
    --project data-aquarium \
    --artifact-base-uri gs://ashare-artifacts/reports/strategy1 \
    --local-mirror-root reports/strategy1 \
    --skip-gcs-upload
...
Cloud Run Python baseline search（PRD-20260605-04）：sklearn native 首轮 Top5 rejected 后，下一轮用 LightGBM 在 Cloud Run task fan-out 上寻找新的 Python baseline。P0 manifest 固定为 `configs/strategy1/cloudrun_python_lgbm_pvfq_n30_bw_h5_v0.yml`：40 个 `lightgbm_gbdt` 候选、默认 20 task 并发、数据截止 `2026-04-30`、固定 `pv_fin_quality + 30/5% + biweekly + 5d`，并使用 `configs/strategy1/model_acceptance_contract_v3.yml` 作为当前 live 写回共享验收契约。历史 v1 只保留为已完成搜索的审计契约。live v3 评估窗口必须来自实际 backtest span / manifest final_holdout window；`model_acceptance_contract_v3.yml` 中的 `default_replay_and_initial_cutover_full_period` 只用于 replay / 初次 cutover fallback，不得无条件套用于未来滚动 live search。

live v3 cutover smoke 验收点：不只检查 registry / summary 是否写出 `v3_*` 字段，还必须确认 live registry row 的信号字段能正确驱动复用的 v3 gate。最小 smoke 应在相同候选上对照 #122 replay 基准，确认 accepted / rejected 分布仍为 `25` 个候选中 `1 accepted / 24 rejected`，并检查 comparison artifact 的 `v3_relative_gate_by_benchmark.csv` 与 registry 中 `v3_passed_benchmark_count`、`v3_acceptance_status` 一致。
...
风险特征入模搜索（PRD-20260606-03）：Cloud Run Python 路线使用 `sql/cloudrun/strategy1/01_build_training_panel.sql` 构建 `strategy1_pv_fin_risk_v0_20260606` 面板；该面板在 `pv_fin_quality` 基础上加入个股风险、市场状态和交互特征，并要求 `dws_market_state_daily` 有显式分区过滤与完整覆盖。P0 拆为两个 search manifest：`configs/strategy1/cloudrun_python_riskfeat_lgbm_pvfq_n30_bw_h5_v0.yml`（20 个 `lightgbm_gbdt`）和 `configs/strategy1/cloudrun_python_riskfeat_lgbm_regression_pvfq_n30_bw_h5_v0.yml`（20 个 `lightgbm_regression`），均固定 `30/5% + biweekly + 5d`、`ledger_exec_v1_lot100`、`test_reuse_wave_no=4`、`final_holdout=2026-01-05..2026-04-30`。Top5 仍先跑 `19_qa_cloudrun_python_baseline_search_outputs.sql`，再跑 `21_qa_risk_feature_search_outputs.sql` 校验 feature schema/delta hash、市场状态覆盖、feature importance、wave 4 复用记录和 `max_drawdown >= -18%` 风险目标。
...
验收门 v3 只读 replay（PRD-20260608-02）：`configs/strategy1/model_acceptance_contract_v3.yml` 是 `v3` benchmark、复利、Sharpe / Calmar、五指数相对门、legacy replay carve-out、replay scope、窗口和 final_holdout enforcement 的唯一事实来源。`scripts/strategy1/replay_acceptance_gate_v3.py` 只读五次正式搜索的 selected model / backtest / NAV / `dwd_index_eod`，不重训模型、不改 historical `accepted/rejected`、不回写 ADS；它输出 `acceptance_gate_v3_replay/` artifact，包括 replay summary、candidate 级结果和逐指数相对门明细。BigQuery 侧不要再直接裸跑 `sql/ml/strategy1/24_qa_acceptance_gate_v3_replay_outputs.sql`；应通过 `scripts/strategy1/run_acceptance_gate_v3_replay_qa.py` 从 contract 渲染 `contract hash`、replay search scope、benchmark 集合、窗口、信号/绝对门阈值、允许的 score orientation、`legacy_valid_as_cv_search_ids` 和 `final_holdout enforcement` 后执行，这样 replay 与 QA 才共享同一 contract 语义。`24` QA 本身不检查 artifact 文件存在性，而是复算 replay 依赖的 contract identity、五次正式搜索 Top-K 覆盖、五指数窗口覆盖、绝对指标公式，以及“direct-pass 不得绕过正超额复合年化收益”这一相对门前提。

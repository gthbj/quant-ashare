...
-- 核心 DIM/DWD 表级与字段级说明补齐脚本。
-- 用法：核心表建表或重建后执行本脚本；本脚本只更新 BigQuery metadata，不重写表数据。
...
ALTER COLUMN sec_type SET OPTIONS (description = '证券品种，当前固定为 stock'),
...
SET OPTIONS (description = '指数主维表。维护 canonical 指数代码、ODS 实际代码、index_daily/index_dailybasic 端点可用性、起止日期和 benchmark 候选标记。');
...
SET OPTIONS (description = '股票日线价格 DWD。以交易日历乘股票生命周期为骨架，默认全量/日常路径写入 2019-01-01 之后数据；owner 显式 backfill 可写入指定历史训练窗口。保留停牌日空行情行，并提供后复权价格、涨跌停与可交易掩码。');
...
SET OPTIONS (description = '股票分红送转事件 DWD。消费键为 (sec_code, ex_date)，同股同除权日多条实施事件按现金分红、送股和转增比例 canonical 聚合。');
...
SET OPTIONS (description = '股票日频估值与股本 DWD。来自 Tushare daily_basic，默认全量/日常路径写入 2019-01-01 之后数据；owner 显式 backfill 可写入指定历史训练窗口。统一输出元和股口径。');
...
SET OPTIONS (description = '指数日线 DWD。从 dim_index 读取 canonical/source 映射和可用端点；sec_code 输出规范指数代码，source_sec_code 保留 ODS 实际代码，并从 index_dailybasic 写入可用指数的估值、市值和股本字段。index_daily 成交量/成交额已统一换算。');

...
def test_weight_book_generates_expected_levels_and_l3_equal_top20() -> None:
    ...
    assert set(book["level"]) == {"L0", "L1", "L2", "L3"}
...
def test_weight_correlation_uses_union_with_missing_as_zero() -> None:
    ideal = {"a": 0.7, "b": 0.3}
    actual = {"a": 0.5, "c": 0.5}

    corr = analysis.weight_correlation(ideal, actual)
...
def test_report_contains_preregistered_rules_and_no_score_reversal_instruction() -> None:
    ...
    tc = pd.DataFrame([{"tc_target": 0.5, "tc_realized": 0.4}])

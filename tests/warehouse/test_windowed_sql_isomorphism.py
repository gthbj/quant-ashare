...
SQL_PAIRS = (
    SqlPair(
        ...
        token_map={
            "dwd_start_date": "__WRITE_START__",
            "p_dwd_write_start_date": "__WRITE_START__",
            "lookback_start_date": "__WRITE_START__",
            "dwd_end_date": "__END__",
            "p_write_end_date": "__END__",
            "p_write_floor_date": "__WRITE_START__",
        },
        regex_substitutions=(
            RegexSubstitution(
                r" AND SAFE\.PARSE_DATE\('%Y%m%d', trade_date\) BETWEEN __WRITE_START__ AND __END__",
                "",
            ...
            RegexSubstitution(
                r", prev_close AS \(SELECT sec_code, ARRAY_AGG\(close_hfq IGNORE NULLS ORDER BY trade_date DESC LIMIT 1\)\[SAFE_OFFSET\(0\)\] AS prev_close_hfq FROM `data-aquarium\.ashare_dwd\.dwd_stock_eod_price` WHERE trade_date BETWEEN __WRITE_START__ AND DATE_SUB\(__WRITE_START__, INTERVAL 1 DAY\) GROUP BY sec_code\)",
                "",
    ...
    SqlPair(
        ...
        + (
            RegexSubstitution(
                r"WITH base AS \(WITH first_write AS \(.*WHERE v\.trade_date BETWEEN __READ_START__ AND __END__\), windowed AS",
    ...
    SqlPair(
        ...
        token_map={
            "dws_start_date": "__START__",
            "p_label_read_start_date": "__START__",
            "p_label_write_start_date": "__START__",
            "dws_end_date": "__END__",
            "p_label_read_end_date": "__END__",
            "p_write_end_date": "__END__",
        ...
        + (
            RegexSubstitution(
                r"DATE_ADD\(__END__, INTERVAL 45 DAY\)",
                "__END__",
                "window refresh reads forward label exits beyond the output write window",
            ),
            RegexSubstitution(
                r" WHERE b\.trade_date BETWEEN __START__ AND __END__",
                "",
...
def test_windowed_sql_select_bodies_match_canonical_sources() -> None:
    ...
    boundaries, parameter naming, and documented refresh-only helper logic.
    """

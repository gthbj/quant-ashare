...
| OQ-010 | P0 策略默认参数待确认：默认调仓频率、持股数/单票权重上限，以及后续特征/标签/选股口径。成本子项已由 `PRD_20260602_02_OQ010交易成本口径.md` 固化为佣金万一免五、卖出印花税 5 bps、买/卖滑点各 5 bps，并已在 runner SQL 中实现；策略报告已由 `PRD_20260602_03_策略1中文报告归因分析.md` 固化为评估主基准中证1000 `000852.SH`、展示对比基准沪深300 `000300.SH`，且报告中文化/交易明细/亏损归因/AI 诊断已在 PR #20 实现；`PRD_20260602_04_策略1模型质量诊断.md` 已定义先诊断 signal/label/sample/universe/portfolio/cost/style，再进入参数和模型实验，诊断 QA（`12`）已通过；`PRD_20260602_05_策略1预测池口径修正.md` 已定义并实现了 valid/test live-available 预测池口径（PR #29/30）；livepool reverse-score shadow run 已显示 raw 正类概率方向存在稳定反向风险，`PRD_20260603_01_策略1分数方向校准.md` 已实现 score orientation 校准（PR #32），oriented run 验证通过；`PRD_20260603_02_策略1首轮质量迭代实验.md` 已提出第一轮分阶段实验矩阵；首个模型训练工具链已定为 BigQuery ML + SQL runner；首个基线股票池已定为仅沪深主板，不含北交所、创业板、科创板。 | open: 成本子项、报告实现、诊断、预测池口径和分数方向校准均已完成；首轮质量迭代实验 PRD 待 owner review，下一步按确认后的矩阵实现实验参数化、manifest、对比报告并执行实验 | owner | docs/数据仓库建模方案-DWS-ADS.md §10; docs/A股中低频小资金机器学习策略方案.md §12; docs/策略1-ml_pv_clf_v0-runner设计.md; docs/prd/PRD_20260601_02_策略1BQML回测闭环.md; docs/prd/PRD_20260602_02_OQ010交易成本口径.md; docs/prd/PRD_20260602_03_策略1中文报告归因分析.md; docs/prd/PRD_20260602_04_策略1模型质量诊断.md; docs/prd/PRD_20260602_05_策略1预测池口径修正.md; docs/prd/PRD_20260603_01_策略1分数方向校准.md; docs/prd/PRD_20260603_02_策略1首轮质量迭代实验.md |

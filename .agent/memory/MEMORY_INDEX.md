...
当前阶段：**DWD/DIM 建模方案已完成，P0 DIM/DWD 已物化并通过 smoke QA；`dwd_index_eod` 估值/股本字段已恢复**。正确口径：财务/事件按分区前移到 2017，行情最终写 2019+ 但构建时读 2018 lookback buffer，维度/日历取最新快照或全量历史事件；下一步是补财务三表或进入 DWS 特征/标签。

...
### 最新补充（2026-06-11）：PRD_02 final refit 已完成，PRD_03 synthetic continuous merge 分区过滤 hotfix 中
...
- Official synthetic merge 首次执行暴露 BigQuery 分区过滤缺口：source prediction join 只有 manifest 动态窗口，未提供可用于 partition elimination 的静态 `predict_date` 过滤，BigQuery job `cde0ff0e-fe0a-4124-89d1-c5406a8c5caa` 失败；目标 registry 可能已有 partial row，prediction 写入未完成。
- 分支 `codex/synthetic-continuous-partition-filter` 正在修复上述缺口：synthetic prediction insert 增加整体 `predict_date` 静态窗口，`qa_continuous_backtest_outputs` 的 source count 也增加 `p_predict_start` / `p_predict_end` 分区过滤；focused tests、`qa_continuous_backtest_outputs` BigQuery dry-run 和 `git diff --check` 已通过。合并后需以 `--force-replace` 重跑 official synthetic merge，再执行 single continuous ledger 与 QA。

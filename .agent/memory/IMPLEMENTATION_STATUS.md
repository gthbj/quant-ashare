...
- Part B transfer ladder：L0 score-weighted long/short no-cost annual Sharpe=`2.800047`，20bps 成本后仍为 `1.807056`，说明信号容量不是主要瓶颈；L1/L2/L3 long-only IR 约 `0.491`/`0.544`/`0.486`，显著低于 L0。L3-L2 IR 差=`-0.058363`，L1-L2 IR 差=`-0.053236`，暂不支持“扩到 Top50”或“等权替代分数权重”作为优先改进。平均 `TC_target≈0`、`TC_realized≈-0.003927`，说明现行 equal Top20 目标/真实持仓几乎不保留 score-weighted 强度。

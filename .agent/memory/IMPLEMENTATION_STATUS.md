...
- OQ-006 接口单位换算口径 PRD 草案已完成并记录 owner 关键决策：`docs/prd/PRD_20260602_01_OQ006接口单位换算口径.md`。PRD 将 OQ-006 从人工记忆升级为单位契约 + 覆盖检查 + DWD 准入门禁；owner 已确认 `ashare_meta.ods_field_unit_map` 为单位换算唯一事实来源、`dwd_index_eod.volume/amount` 当前仍是 `index_daily` 源单位手/千元且必须按 `vol*100` / `amount*1000` 换算并迁移为 `volume_share/amount_cny`、OQ-006 最小实现先于 P1 高单位风险 DWD 正式落地、`sql/qa/05_oq006_unit_checks.sql` 加入所有 DWD PR 必跑 QA。OQ-006 仍 open，待实现。
...
- OQ-006 实现：新增 `ashare_meta.ods_field_unit_map`，补 P0 + PR #13 财务三表首批单位映射 seed，修复 `dwd_index_eod.volume/amount` 未换算问题并迁移为 `volume_share/amount_cny`，新增 `sql/qa/05_oq006_unit_checks.sql` 并纳入 DWD PR 必跑 QA，将单位准入硬规则写入 DWD-DIM / README / `KNOWN_CONSTRAINTS.md`。

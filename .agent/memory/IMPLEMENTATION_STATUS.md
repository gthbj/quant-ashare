...
项目处于**P0 DIM/DWD 已物化并通过 smoke QA，OQ-004 基准指数代码可用性已实现并关闭，策略 1 价格量价 DWS/ADS SQL 已物化并通过 QA，策略 1 BigQuery ML runner `sql/ml/strategy1/01-10` 已于 PR #12 端到端实跑并通过全部 QA，OQ-006 接口单位换算已实现并关闭（PR #16），OQ-003 财务报表口径已采纳、关闭并实现（PR #13：三大报表 DWD + `_latest` + `dws_stock_feature_fin_daily` 已物化并通过 `sql/qa/04`，并按 OQ-006 在 `ods_field_unit_map` 补全财务字段映射、跑通 `sql/qa/05`），OQ-010 交易成本子项已形成 PRD 并在 runner SQL 中实现（佣金万一免五、卖出印花税 5 bps、买/卖滑点各 5 bps），策略 1 中文报告与归因分析已实现并合并（PR #20：报告中文化、评估主基准中证1000 `000852.SH`、展示对比基准沪深300 `000300.SH`、交易/持仓/NAV 附件、亏损证据包和 AI 诊断），策略 1 报告 GCS uploaded 模式已跑通（`gs://ashare-artifacts` 已创建，本机 ADC 已配置，真实 `report_uri` 已回写 ADS，runner QA 全过），策略 1 模型质量诊断 PRD 已新增（`PRD_20260602_04_策略1模型质量诊断.md`），valid/test live-available 预测池口径修正 PRD 已新增（`PRD_20260602_05_策略1预测池口径修正.md`），并已基于 livepool reverse-score shadow run 新增策略 1 分数方向校准 PRD（`PRD_20260603_01_策略1分数方向校准.md`）**阶段。已产出 DWD/DIM 建模方案、DWS/ADS 表设计方案、策略方案、策略 1 PRD、runner 设计与 runner 实现 PRD、OQ-003 财务 `report_type` 口径 PRD、OQ-004 基准指数代码可用性 PRD、OQ-006 单位契约 PRD、OQ-010 交易成本口径 PRD、策略 1 中文报告与归因分析 PRD、策略 1 报告 GCS 上传运行手册、策略 1 模型质量诊断 PRD、策略 1 预测池口径修正 PRD、策略 1 分数方向校准 PRD。owner 澄清：财务/事件按分区前移到 2017，行情最终写 2019+ 但构建时读 2018 lookback buffer，维度/日历取最新快照或全量历史事件；OQ-003 已采纳 P0 默认合并报表 `report_type='1'`、DWD 保留口径字段、DWS 默认过滤默认口径。根目录 `sql/` 已覆盖 P0 DIM/DWD、财务三大报表 DWD、策略 1 DWS（含财务特征）、策略 1 ADS 表契约、策略 1 BigQuery ML runner、metadata、QA 和 OQ-006 单位契约；BigQuery 目标已建 4 张 DIM + 5 张 P0 DWD + 6 张财务三大报表 DWD（income/balancesheet/cashflow + 各自 `_latest`）+ 7 张策略 1/通用 DWS（含 `dws_stock_feature_fin_daily`）+ 11 张 ADS 契约表，并新增 `ashare_meta.ods_field_unit_map` 单位契约表。OQ-004 已新增并物化 `dim_index`，`dwd_index_eod` 已改为从 `dim_index` 读取映射并重建，`sql/qa/03_oq004_index_checks.sql` 通过，runner 08 benchmark 前置校验 dry-run 通过；PR #11 review feedback 已跟进并已合并到 `main`，合并后本地/远端 `codex/implement-oq004-index` 分支已删除。OQ-007 的 ODS `stock_basic_delisted.delist_date` 已复核为 `STRING` 且可解析，仓库 SQL 已改为优先使用 ODS 退市日，PR #9 已合并；2026-06-02 已重建 `dim_stock`、`dwd_stock_eod_price`、策略 1 DWS 六表与 ADS 契约表，并重新执行 metadata、P0 QA 和策略 1 QA。暂无调度代码。策略 1 runner 已于 PR #12 在 BigQuery 端到端实跑并通过全部 QA：run_id `s1_bqml_20260601_01` / backtest `bt_s1_bqml_20260601_01`，产出训练面板 3,051,752 行、5 个 BQML LOGISTIC_REG 候选+选型、预测/候选/组合/订单/回测/监控 ADS 结果和本地报告，`10_qa_runner_outputs.sql` 16 个断言全过；2026-06-02 已按 `docs/策略1报告GCS上传运行手册.md` 去掉 `--skip-gcs-upload` 重跑 `render_report.py`，GCS 报告路径为 `gs://ashare-artifacts/reports/strategy1/ml_pv_clf_v0/run_id=s1_bqml_20260601_01/backtest_id=bt_s1_bqml_20260601_01`，ADS `report_upload_status=uploaded`，`10_qa_runner_outputs.sql` 通过。模型质量诊断主流程已对同一 run/backtest 执行并上传 GCS，ADS 已回写诊断字段，主结论为 `sample_filter_risk` high（test trainable ratio 0.44，valid/test RankIC 仍为负）；但 `sql/ml/strategy1/12_qa_model_diagnosis_outputs.sql` 当前因 `split_tag` 列歧义未通过 QA。实跑暴露并修复多处运行期问题（03/07/08/09 相关子查询去相关 + 分区过滤、10 `cal_date`、render ADC 回退 + PARSE_JSON wide number），并按 DECISION-20260601-07 将 08 由 v0 set-based 重写为账户级有状态 ledger（v0 守卫 cash/gross 在真实数据上失败）。livepool reverse-score shadow run 已显示当前 raw 正类概率方向存在稳定反向风险，新增 PRD 要求后续实现 `raw_score`、oriented `score` 与 `score_orientation`，在 03 选型登记方向、04 预测阶段应用方向；该实现尚未开始。

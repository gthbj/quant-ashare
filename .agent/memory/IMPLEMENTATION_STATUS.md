...
- OQ-010 策略 1 实验并发调度与隔离 Phase 1 已实现（PR #?）：新增 `sql/meta/03_strategy1_experiment_run_status.sql`（状态表 DDL）、`scripts/strategy1/run_oq010_experiments.py`（调度器，支持 --dry-run 展开完整计划、GCS `ifGenerationMatch=0` 原子锁、lease/heartbeat/stale reclaim、全部 PRD 定义 CLI 参数）、`sql/qa/06_strategy1_experiment_concurrency_checks.sql`（12 个 QA-CONC 断言）、`docs/策略1实验并发调度器运行手册.md`。验证：Python `py_compile`、`git diff --check`、dry-run 展开 stage_a 多实验计划通过。尚未在 BigQuery 端到端实跑并发实验。
...
- OQ-010 策略 1 实验并发调度与隔离 Phase 1（状态表、调度器 dry-run、GCS 原子锁、并发 QA）已实现并通过静态验证；Phase 2-4（portfolio-only 并发执行、08 ledger 并发、retrain 混合队列）待实现和验收。

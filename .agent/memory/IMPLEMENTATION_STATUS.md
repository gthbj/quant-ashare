...
- OQ-010 Stage C runner/QA 修复已由 PR #47 合并后完成重跑；随后补齐 3*2*2*2 全因子网格中缺失的 19 个组合，24 个组合最终均通过 `12_qa_model_diagnosis_outputs`。全网格当前最优组合为 `pv_fin_quality + 30/5% + biweekly + 5d`（benchmark=`000852.SH`，total_return=41.10%、excess_return=12.09%、Sharpe=1.043、max_drawdown=-14.48%）。本轮实跑发现并本地修复两项调度/诊断问题：同 stage 依赖分批需尊重 `depends_on_experiment_id`，诊断完成状态需与 GCS 上传状态拆分；待单独提 PR 合入 `main`。

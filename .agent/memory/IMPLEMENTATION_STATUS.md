...
- 策略 1 报告 GCS uploaded 模式运行手册已新增：`docs/策略1报告GCS上传运行手册.md`。手册覆盖 `ashare-artifacts` bucket、ADC、IAM、local-only smoke、uploaded 模式运行、GCS/ADS/QA 验收和安全约束；尚未实际创建 bucket 或重跑 uploaded 模式。
...
- `TODO.md` 已整理为下一步可执行清单：已移除 PR #13 / OQ-003 待合并旧项，当前仅保留 `dws_market_state_daily`、OQ-010 策略质量、GCS 报告、P1 扩展与工程/调度待办；开放问题继续以 `OPEN_QUESTIONS.md` 为唯一来源。
...
- 策略 1 runner v0 模型质量提升（独立于管线）：当前 `ml_pv_clf_v0` valid rank_ic≈-0.10（反向预测）、回测 NAV≈0.02，需要特征/标签/选股口径迭代（OQ-010），管线本身已端到端跑通。OQ-010 成本 profile 与中文报告已实现；剩余重点是模型质量和参数。

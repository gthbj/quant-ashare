...
- OQ-010 首轮实验 runner 实现 PR #37 已合并并验证 dry-run；review 后已补 portfolio-only `prediction_run_id` 路径和目标权重留现金口径：等待执行第一轮实验。

...
- Ledger v1 与月度滚动重训目前仅完成 PRD；实现顺序必须先 Ledger v1 A/B，再月度重训，避免交易执行语义变化和模型生命周期变化混在一起。

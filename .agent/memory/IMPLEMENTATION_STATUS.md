...
- 策略 1 runner 已在 BigQuery 端到端实跑并通过全部 QA（PR #12）：run_id `s1_bqml_20260601_01` / backtest `bt_s1_bqml_20260601_01`。01 训练面板 3,051,752 行；02 训练 5 个 BQML LOGISTIC_REG 候选；03 选 `l1_0_l2_1e_3`（valid rank_ic≈-0.10、AUC≈0.50，反向预测基线）；04 预测 1,052,687 行；05 候选 224,648 行；06 组合 520 行；07 订单 565 行；08 ledger 回测（NAV 485 天）；09 summary+monitor；render 本地报告 + 回写 report_uri；`10_qa_runner_outputs.sql` 16 个断言全过。实跑修复：03/07/08/09 BigQuery 相关子查询去相关 + 强制分区过滤、10 `cal_date`、render ADC 回退 + PARSE_JSON wide number；08 因 v0 set-based 守卫失败（现金 -34 万、gross 2803 倍）按 DECISION-20260601-07 重写为账户级有状态 ledger。NAV 收于≈0.02 属 v0 模型质量（OQ-010），非管线缺陷。

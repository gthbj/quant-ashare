...
### 最新补充（2026-06-29）：CLAUDE.md CodexRadar authenticated API 入口更新

- 把 `CLAUDE.md` 的 Codex「模型 + 思考强度」取数入口从旧 `current.json` 改为 authenticated `https://codexradar.com/api/v1/current`；`x-api-key` 通过本机环境变量 / secret 注入，推荐 `CODEXRADAR_API_KEY`，key 值不写入仓库、记忆、日志或 PR comment。选择语义保持 `DECISION-20260628-01`：按 `model_iq` 最高 `score` 选择 `model + reasoning_effort`，同分先版本更低、同模型内再强度更低。仅文档/记忆改动，无代码/SQL/数据/生产变更。决策见 `DECISION-20260629-01`。

### 最新补充（2026-06-28）：CLAUDE.md Codex 模型/强度选取升级对齐 quant-crypto

- 把 `CLAUDE.md` 的 Codex「模型 + 思考强度」选取逻辑从「固定 `gpt-5.5` + 固定 `xhigh`」升级为请求 codexradar `current.json` 动态选当日 model_iq 智商最高的 model+effort；同分最高并列时先选版本更低的模型、再选思考强度更低的一档（low>medium>high>xhigh），新增 bash 解析片段 + CLI 显式传 `-m`/`-c`，请求失败/缺数据报错暂停。仅文档/记忆改动，无代码/数据/生产变更。决策见 DECISION-20260628-01；原在 feature 分支 `experiment/largecap-value-results` 提交，2026-06-28 cherry-pick 进 main。

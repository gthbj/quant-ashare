...
- OQ-006 实现：新增 `ashare_meta.ods_field_unit_map`，补 P0 + PR #13 财务三表首批单位映射 seed，迁移 `dwd_index_eod.volume/amount` 为 `volume_share/amount_cny`，新增 `sql/qa/05_oq006_unit_checks.sql` 并纳入 DWD PR 必跑 QA，将单位准入硬规则写入 DWD-DIM / README / `KNOWN_CONSTRAINTS.md`。

...
- OQ-006 接口单位换算口径实现中（PR #16）：`ashare_meta.ods_field_unit_map` 已创建并填充 P0 + PR #13 首批单位映射；`dwd_index_eod.volume/amount` 已按 `vol*100` / `amount*1000` 修复换算并迁移为 `volume_share/amount_cny`；`sql/qa/05_oq006_unit_checks.sql` 已创建并纳入 DWD PR 必跑 QA；单位准入硬规则已写入 DWD-DIM §3.3-H 和 `KNOWN_CONSTRAINTS.md`。PR #16 review 反馈：财务金额字段需补充 `legacy_unsuffixed` 命名例外、QA-UNIT-6e 需改为 ODS 直接比对、OQ-006 状态待 BQ 验证通过后关闭。
...
- OQ-006 实现中（见上），待 PR #16 合并并通过 BQ 验证后关闭。

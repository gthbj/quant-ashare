...
2026-06-06 OQ-005 Composer DAG 拆分实现已完成到本地可验证阶段，尚未部署 Composer。新增 `orchestration/composer/dags/ashare_common.py` 作为共享 helper，新增 production DAG `ashare_ods_ingestion_daily`、`ashare_warehouse_window_refresh`、`ashare_warehouse_full_rebuild`；旧 `ashare_daily_pipeline_v0` 保留为迁移期回滚入口。P0 跨 DAG 触发先使用 `TriggerDagRunOperator`，并通过 `pipeline_run.upstream_pipeline_run_id` / `triggered_by_dag_id`、`v_pipeline_refresh_missing` 和 `warehouse_refresh_missing` 告警补可观测链路。PR #86 review follow-up 已补：`ashare_ods_ingestion_daily` 新建默认 paused，迁移时先暂停旧 scheduled DAG 再 unpause 新 scheduled DAG；`ashare_warehouse_window_refresh` 串行执行；refresh-missing 只在完全没有 linked warehouse run 时触发，已触发但失败的下游 run 交给 failure 告警。`ashare_warehouse_window_refresh` 保留 `qa_only` 只读 QA 能力；生产启用前必须完成 ingestion→warehouse、非交易日 skip、backfill、qa_only 和 refresh-missing smoke。

...
当前 Phase D2 已把普通 Strategy1 执行切为 research-first。`configs/strategy1/active_step_catalog.yml` 的 `current_dataset_role` 为 `research`，`research.enabled_by_default=true`；SQL render、runner、SQL runner、report、diagnosis、QA、acceptance/comparison 和 factor attribution 默认解析到 `ashare_research.research_*`。显式 `--output-dataset-role ads` / `dataset_role="ads"` 保留给历史 ADS audit；ADS 模式下 `experiment_run_status` 仍通过 per-role dataset override 解析到 `data-aquarium.ashare_meta.strategy1_experiment_run_status`。Catalog 会用 pytest 校验每个非 retired step 的 `inputs` / `outputs` 覆盖 SQL 中实际 `ashare_ads` FQN，且 research 渲染结果不得残留 `data-aquarium.ashare_ads.`。无 step 上下文的全局 research 替换仍必须 fail-fast 或排除歧义 role，避免 `ads_model_registry` 同时对应 `model_registry` / `acceptance_result` 时误替换。普通 research 输出 lifecycle 默认值为 `research_status='candidate'`、`promotion_status='not_promoted'`；promotion manifest 自身默认 `promotion_status='planned'`。

Phase D3 已新增显式 owner-approved promotion job：`quant_ashare.strategy1.promotion` / `python -m scripts.strategy1.promote_research_to_ads`。Promotion 默认要求 accepted research、显式 approval metadata、source run/backtest/model 和 date window；默认复制正式 publishable outputs 到 ADS 并写 `research_promotion_manifest`，不默认复制 training panel，不追溯迁移历史 ADS。CLI 真实写 ADS 必须显式传 `--execute`；`--print-sql` / 默认模式只做 review-only 输出。Promotion 只更新 promotion lifecycle 字段，不反写 `acceptance_status='accepted'` 或 `research_status='accepted'`；即使 owner 显式 `--allow-unaccepted`，验收状态也只能由 acceptance 流程写入。普通 runner/report/QA/acceptance 仍不得隐式写 ADS。

Phase E 当前包边界为 `src/quant_ashare/strategy1/**`：dataset routing、acceptance、ledger、reporting/backtest、pipeline-control/orchestrator 和 promotion 已有 package modules；旧 `scripts.strategy1_cloudrun.*` 对应文件仅保留兼容 wrapper，直到 Cloud Run entrypoint 单独迁移并 smoke。
...
2026-06-07/08 已补齐上证指数 `000001.SH` 的生产 BigQuery 读链路：`index_daily_000001_SH` / `index_dailybasic_000001_SH` 已加入 current-scope manifest、ODS external table 显式 `sourceUris`、`dim_index` seed 和 `dwd_index_eod`。由于 ODS index external tables 使用显式 URI 列表，不是 `endpoint=*` 自动发现，新增 index endpoint 时必须先更新 `configs/ingestion/ods_current_scope_v0.yml`，再运行 `scripts/ingestion/generate_index_external_table_uris.py` 生成 `sql/ods/01_index_external_table_uris.sql`，并可用 `--check` 防止 SQL 与 manifest 漂移；否则 GCS 已有文件也不会被 BigQuery ODS 读取。`ashare_warehouse_window_refresh` 的窗口转换链路已补 `sql/incremental/02_refresh_index_dwd_window.sql`，后续 daily_current/backfill 会先刷新指数 DWD，再刷新股票 DWD/DWS。随后按 owner 要求补 DWS：已创建 `data-aquarium.ashare_backup`，将修改前 `ashare_dws.dws_market_state_daily` 备份为 `ashare_backup.dws_market_state_daily_v0` 并写入表说明；生产 `dws_market_state_daily` 同时输出 `market_state_v0_20260606` 兼容行和 `market_state_v1_20260607` 行。v0 的 `sse_composite_*` 字段保持 `NULL`，v1 填充上证指数 `000001.SH` / `SSE_COMPOSITE` 收益、回撤、波动和均线偏离字段，risk-off 触发语义暂不改变。Composer window refresh 已在股票 DWS 刷新与窗口 QA 后接入 `sql/incremental/03_refresh_market_state_window.sql` 和 `sql/qa/11_market_state_checks.sql`；全量 `sql/dws/08_dws_market_state_daily.sql` 只作为初始化 / full rebuild 路径。
...
- `sql/dwd/12_dwd_stock_dividend_event.sql` 是 PRD_20260612_02 Phase A 的分红送转事件 DWD，消费键为 `(sec_code, ex_date)`，对 `ods_tushare_dividend` 已实施事件做 canonical 聚合；`sql/qa/14_corporate_action_event_checks.sql` 是对应公司行为事件 QA，落 `ashare_meta.qa_stock_dividend_event_hfq_mismatch` 双向 mismatch 明细，并生成 `ashare_dwd.v_dwd_stock_dividend_event_ledger_consumable`。
...
- `ashare_warehouse_full_rebuild` now uses `ashare-pipeline-control /v1/tasks/bigquery/submit` + `/v1/tasks/bigquery/poll` for long-running BigQuery steps, instead of synchronous `job.result()`. This keeps Workflows HTTP steps under the `1800s` limit while preserving explicit `pipeline_task_status` writes and the shared warehouse-write lock. The workflow remains a manual opt-in deployment path via `DEPLOY_FULL_REBUILD=true`; this turn validated the async control plane directly and ran only a workflow `pipeline_dry_run=true` smoke, not a destructive full warehouse rebuild.

...
Phase D3 已新增显式 owner-approved promotion job：`quant_ashare.strategy1.promotion` / `python -m scripts.strategy1.promote_research_to_ads`。Promotion 默认要求 accepted research、显式 approval metadata、source run/backtest/model 和 date window；默认复制正式 publishable outputs 到 ADS 并写 `research_promotion_manifest`，不默认复制 training panel，不追溯迁移历史 ADS。普通 runner/report/QA/acceptance 仍不得隐式写 ADS。

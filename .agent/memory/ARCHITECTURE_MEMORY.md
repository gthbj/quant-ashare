...
当前 D0 只定义契约：不部署 BigQuery 对象、不切 runner 默认写入、不迁移历史 ADS、不实现 promotion job。`configs/strategy1/active_step_catalog.yml` 记录 research contract SQL；resolver 支持 per-role dataset/project override，当前 ADS 模式 `experiment_run_status` 解析到 `data-aquarium.ashare_meta.strategy1_experiment_run_status`，不再误拼到 ADS。Phase D1a 已把 table role resolver 接入 `src/quant_ashare/strategy1/sql_render.py`：默认仍渲染 ADS；显式 `dataset_role="research"` 必须传 `allow_future_research=True`，并按 catalog step 的 `inputs` / `outputs` 只改写当前 step 相关表。PR #142 review follow-up 后，catalog 会用 pytest 校验每个非 retired step 的 `inputs` / `outputs` 覆盖 SQL 中实际 `ashare_ads` FQN，且 research 渲染结果不得残留 `data-aquarium.ashare_ads.`。Phase D1b 已为 Cloud Run Python runner、ledger、orchestrator status、report、diagnosis、QA、acceptance/comparison 和 factor attribution 接入显式 `output_dataset_role`；默认仍写/读 ADS 且默认 ADS 子命令不向旧 Cloud Run 镜像下发新增 flag，显式 `research` 模式才把 run-scoped 表指向 `ashare_research.research_*`，其中 research status 表为 `ashare_research.research_experiment_run_status`。普通 research 输出 lifecycle 默认值为 `research_status='candidate'`、`promotion_status='not_promoted'`；promotion manifest 自身默认 `promotion_status='planned'`。D1b 不部署 BigQuery / Cloud Run、不切 default research-first、不实现 promotion；D2 前必须先完成 D1 收尾验收（部署 D0 DDL、补 runtime SA 写权限、重建镜像、真实 research smoke），D3 才实现 owner-approved promotion 到 ADS。

...
- OQ-005 Cloud Run Job 模板仍默认 `--dry-run`，生产写入只能由 Composer 在 `ashare_pipeline_dry_run=false` 时显式传入 `--allow-gcs-write`，不得在脚本里硬编码 token 或绕过 dry-run guard。当前生产入口为 `ashare-ingest-current-scope` 单 execution 顺序执行 14 个 ODS endpoint，并通过 Direct VPC egress + Cloud NAT 固定出口；已完成 `2026-05-20` 至 `2026-06-04` 当前范围 ODS 生产 GCS 写入和 `sql/qa/09_ods_daily_partition_readiness.sql` 验证。每日 DAG 默认主链只读业务日分区或窗口影响范围，不得默认执行 2019+ 全历史 schema 检查或完整 ODS→DIM/DWD/DWS/ADS 重建；`pipeline_dry_run=true` 时窗口分支不得写 DIM/DWD/DWS。`warehouse_mode=daily_current` / `backfill` 的 BigQuery SQL 兼容路径只允许刷新 DIM 小表、恢复 P0 metadata、按 `sql/incremental/01_refresh_stock_dwd_dws_window.sql` 窗口化刷新股票 DWD 与策略 1 DWS，并执行 `sql/qa/10_windowed_stock_refresh_checks.sql`；`daily_current` 若 `date_to` / `business_date` 落在非交易日，窗口 SQL 和 QA 必须先归一到不晚于请求日期的最近 SSE 开市日，`backfill` 保持显式日期口径；窗口 QA 的估值覆盖范围在 `daily_current` 默认使用最近 20 个交易日，在 `backfill` 使用实际写入窗口；最终宽表的 `has_valuation_data` 只按 price-driven feature universe 内 `total_mv_cny/circ_mv_cny` 非空行断言，不要求所有 valuation 行进入宽表。窗口 DML 使用 BigQuery transaction 包裹，日常小窗口失败时整体回滚，大区间 backfill 按年/季/月分块执行；价格特征读取 60 个交易日，估值特征按每只股票写入窗口首日前的实际 60 条估值观测推导读取边界，标签/宽表/样本写入向前回补 20 个交易日。窗口 SQL 与 canonical full SQL 双实现并存期间，发布前或定期必须运行 `scripts/qa/run_windowed_refresh_equivalence.py`，用 scratch shadow 表对比 full path 与 window path 的窗口数值等价；等价 QA 必须校验 `build_start_date` 足够早，避免 full/window shadow 被同样截断后假通过。ADS runner/backtest/report 产物不得由每日 DAG 自动写入。现有 CTAS 全量转换只能通过 `warehouse_mode=full_rebuild` 或 `warehouse_mode=full_rebuild_compat` 显式手工进入，legacy `ashare_enable_full_refresh=true` 必须记录为 `full_rebuild_compat`，不得伪装成 `daily_current` 增量。`warehouse_mode=qa_only` 只能执行只读 QA；ADS 契约初始化只能通过 `enable_ads_contract_init=true` 手工启用。PR #70 合并后 Composer bucket SQL 已与 `main` 同步；非交易日 daily_current 若归一到的最近交易日 ODS 未采集，应在 `ods_daily_partition_readiness` 阶段失败并阻断窗口写入。完整 Dataform 生产链路、告警和补跑验收仍未完成。
...
- 2026-06-05 owner 决策：策略 1 后续执行层不再使用 BQML 或 `sql/ml/strategy1` SQL runner 作为训练、预测、回测、报告、诊断、月度滚动重训或多实验搜索的默认 / fallback / 新增开发路线。既有 BQML / SQL runner 文件和运行结果仅作为 historical reference / audit；若为了回归审计短暂运行，必须显式标记为历史对照，不得登记为新的默认 baseline。BigQuery SQL 仍用于 ODS→DIM/DWD/DWS/ADS 数仓转换、metadata、单位契约、QA、状态表和只读分析；“停止 SQL runner”只特指停止策略模型/回测 runner 路径。
- 策略 1 Cloud Run 训练回测执行器按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 实现：scikit-learn / 后续 Python 模型库替代策略模型训练 / 预测，不替代 BigQuery DWS/ADS、GCS artifact、报告诊断和 QA；P0 仍需 BigQuery / BigQuery Storage / GCS 客户端、`pyarrow`、`polars` 或 `pandas`、`joblib` 等依赖。Cloud Run Python ledger 必须实现同一 `ledger_exec_v1` 交易语义，并与历史 BigQuery ledger 做一致性或差异说明验收。Cloud Run 多实验 orchestrator 的默认并发数必须等于 manifest 可执行实验数量；`--max-parallel-experiments` 未设置或为 0 时不得隐式降到 2、1 或其他保守默认值，owner 显式传 N 时才限流。sklearn P0 默认 `class_weight=None`，正则网格按 sklearn 原生 `C` / `penalty` / `l1_ratio` 重新定义；后续 accepted baseline 由 Cloud Run / native acceptance gate 决定，不再以 BQML parity 作为硬门槛。

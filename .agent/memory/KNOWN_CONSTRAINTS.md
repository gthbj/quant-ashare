...
- OQ-005 Cloud Run Job 模板仍默认 `--dry-run`，生产写入只能由 Composer 在 `ashare_pipeline_dry_run=false` 时显式传入 `--allow-gcs-write`，不得在脚本里硬编码 token 或绕过 dry-run guard。当前生产入口为 `ashare-ingest-current-scope` 单 execution 顺序执行 14 个 ODS endpoint，并通过 Direct VPC egress + Cloud NAT 固定出口；已完成 `2026-05-20` 至 `2026-06-04` 当前范围 ODS 生产 GCS 写入和 `sql/qa/09_ods_daily_partition_readiness.sql` 验证。每日 DAG 默认主链只读业务日分区或近期小窗口，不得默认执行 2019+ 全历史 schema 检查或完整 ODS→DIM/DWD/DWS/ADS 重建；Phase 2.0 现有 CTAS 转换只能通过 `warehouse_mode=full_rebuild` 或 `warehouse_mode=full_rebuild_compat` 显式手工进入，legacy `ashare_enable_full_refresh=true` 必须记录为 `full_rebuild_compat`，不得伪装成 `daily_current` 增量。`warehouse_mode=qa_only` 只能执行只读 QA；ADS 契约初始化只能通过 `enable_ads_contract_init=true` 手工启用。完整 Dataform 生产链路、增量影响窗口、告警和补跑验收仍未完成。

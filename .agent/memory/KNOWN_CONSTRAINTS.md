...
- 策略 1 Cloud Run 训练回测执行器按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 实现：scikit-learn 只替代 BQML `LOGISTIC_REG` 的训练 / 预测，不替代 BigQuery DWS/ADS、GCS artifact、报告诊断和 QA；P0 仍需 BigQuery / BigQuery Storage / GCS 客户端、`pyarrow`、`polars` 或 `pandas`、`joblib` 等依赖。Cloud Run Python ledger 必须实现同一 `ledger_exec_v1` 交易语义，并与当前 BigQuery ledger 做一致性验收。Cloud Run 多实验 orchestrator 的默认并发数必须等于 manifest 可执行实验数量；`--max-parallel-experiments` 未设置或为 0 时不得隐式降到 2、1 或其他保守默认值，owner 显式传 N 时才限流。sklearn P0 默认 `class_weight=None`，正则网格按 sklearn 原生 `C` / `penalty` / `l1_ratio` 重新定义；Cloud Run selected model 必须通过 BQML baseline 模型质量对等门槛，否则标记 `model_quality_not_equivalent`，不得静默沿用 BQML baseline 参数结论。
- 2026-06-05 策略 1 Cloud Run 真实 smoke 约束：训练/预测 job 与回测/报告 job 至少按 16Gi/4CPU、`--max-retries=0` 部署；runtime service account 必须具备源数据读取、BigQuery jobUser、`ashare_ads` dataset 写权限、`gs://ashare-artifacts` objectAdmin 和日志写入权限。容器镜像需安装 CJK 字体（当前使用 `fonts-noto-cjk`）并在 `render_report.py` 中配置可用中文字体候选，否则中文报告图表会出现 glyph missing。真实 smoke `s1_cloudrun_sklearn_smoke_20260604_02` 可运行但 `model_quality_status=model_quality_not_equivalent`，因此 Cloud Run sklearn backend 尚不能作为 BQML 正式 baseline 的等价替代；正式替代前必须通过 parity gate，或由 owner 明确采纳新的 sklearn baseline。
- 2026-06-05 sklearn native search 约束：当前 `asia-east2` Cloud Run 配额已提升到约 40 vCPU / 160Gi，`strategy1-train-candidate-fanout-job` 可按 `1 CPU / 4Gi`、`--tasks=36` 并发跑第一轮 36 个 LogisticRegression 候选；更大规模（如 100 候选）仍应按配额和实际 OOM 情况设置 owner 显式 `--candidate-parallelism`。sklearn native baseline 不再要求通过 BQML parity gate，但必须保留 BQML reference 证据，只能按 `docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md` 的 valid-only Top 5、native acceptance gate、`test_reuse_wave_no` / owner approval / final holdout 规则决定是否 accepted。

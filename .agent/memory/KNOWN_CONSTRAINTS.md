...
- OQ-010 实验并发调度与隔离 Phase 1 已实现：状态表 DDL（`sql/meta/03_strategy1_experiment_run_status.sql`）、调度器（`scripts/strategy1/run_oq010_experiments.py`）、GCS 原子锁（`ifGenerationMatch=0`）、并发 QA（`sql/qa/06_strategy1_experiment_concurrency_checks.sql`）。调度器引入的新约束：GCS 锁对象在 `gs://ashare-artifacts/locks/strategy1/oq010/` 下，默认 TTL 30 分钟，heartbeat 每 60 秒刷新；stale reclaim、heartbeat 写入与 release 必须使用已检查/持有的 GCS object generation 做条件操作，禁止无条件删除同名 lock object；状态表只用于审计和 resume 输入，不承担锁管理职责，但 `lock_expires_at` / `last_heartbeat_at` 必须与 GCS lock lease 对齐。Phase 2-4（portfolio-only 并发执行、08 ledger 并发、retrain 训练锁）仍待实现和验收；在当前约束下，本机本地多进程裸跑 SQL 仍被禁止，必须通过调度器 / 锁 / 状态表执行。“不并发”约束执行；只有状态表、GCS 原子锁、lease/heartbeat、调度器、写隔离、单实验 QA 和并发串号 QA 实现并验收后，才允许同阶段 portfolio-only / retrain 实验按配置并发。

...
7. Promotion CLI 默认 review-only；真实写 ADS 与 manifest 必须显式传 `--execute`，`--print-sql` 单独使用不得执行写入。
8. Promotion 只更新 promotion lifecycle 字段。`--allow-unaccepted` 只表示 owner 显式绕过 accepted guard，不得把 `acceptance_status` 或 `research_status` 改写为 `accepted`。
9. Promotion 必须检查 source backtest trade / NAV 对同一 `backtest_id` 没有 promotion window 外行，并检查 summary 日期范围被 promotion window 覆盖，避免短窗口静默部分发布。
10. 本次 PR #150 同时交付 D3 promotion job 与 Phase E package 迁移，是 owner 基于连续重构上下文接受的一次性边界豁免；后续 Cloud Run entrypoint 迁移仍需单独 PR。
...
4. Promotion ASSERT 失败会因 BigQuery transaction 回滚而不留下 `research_promotion_manifest` 成功行；失败 attempt 审计依赖 BigQuery job history / Cloud Run execution logs。
...
## DECISION-20260612-01: 采纳 true-five-year continuous 为研究 baseline，关闭 OQ-011

...
- `Excess Calmar Ratio` 改为每个完整自然季度、逐指数计算，使用策略超额季化收益除以该季度策略最大回撤同期超额绝对值，要求每个完整季度相较五个指数至少一个 `quarter_excess_calmar_ratio > 0.9`。
- v3 增加策略超额季化收益门：每个完整季度相较五个指数至少一个 `strategy_quarterized_excess_return > 0`。

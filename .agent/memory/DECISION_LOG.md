...
## DECISION-20260607-02: 策略验收门 v3 改为完整季度季化门
...
owner 继续收敛策略 1 验收门 v3：不再使用 IR；收益与风险门只保留 Sharpe 和 Calmar；final holdout 只保留交易日数；主 benchmark 改为上证指数；周期化收益需严谨区分标准换算因子和实际交易日数。进一步要求 Excess Calmar、Calmar 和策略超额收益都按完整季度逐季比较。

### 决策

- `Sharpe` 门槛改为 `>= 0.90`。
- `Calmar Ratio` 改为每个完整自然季度单独计算，使用策略季化收益除以该季度策略最大回撤绝对值，要求每个完整季度 `quarter_calmar_ratio > 0.9`。
- `Excess Calmar Ratio` 改为每个完整自然季度、逐指数计算，使用策略超额季化收益除以该季度策略最大回撤同期超额绝对值，要求每个完整季度相较五个指数中至少一个指数 `quarter_excess_calmar_ratio > 0.9`。
- v3 增加策略超额季化收益门：每个完整季度相较五个指数中至少一个指数 `strategy_quarterized_excess_return > 0`。
- 为避免跨指数拼接，推荐同一完整季度由同一个指数同时满足策略超额季化收益和 Excess Calmar Ratio。
- 不满完整自然季度的区间不参与季度门，例如回测截至 `2026-04-30` 时，`2026Q2` 只覆盖 4 月，不参与季度对比。

### 理由

完整季度门能降低全周期均值掩盖局部失效的风险；季化收益使用复利换算，保持与项目周期化收益默认口径一致；不完整季度样本不足，纳入逐季门会引入噪声和不可比性。

### 影响

`docs/prd/PRD_20260607_01_策略1验收门v3.md` 已按完整季度季化门重写相关公式、阈值、contract 示例、QA 要求和输出物要求。后续 v3 实现必须支持完整季度识别、季化收益、季度 Calmar、季度 Excess Calmar 和不完整季度排除原因。

### 备选方案

继续使用全周期 Calmar / Excess Calmar，或纳入不完整季度做月内比较；均不采纳。

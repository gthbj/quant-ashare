...
- `Excess Calmar Ratio` 定义为 `策略超额复合年化收益 / abs(策略最大回撤同期超额)`；通过指数必须同一指数下 `策略超额复合年化收益 > 0`，然后满足 `Excess Calmar Ratio > 1` 或 `策略最大回撤同期超额 > 0`。
- `Excess Calmar Ratio` 的指数集合为上证50、沪深300、中证1000、上证指数、深证成指，五个指数任一满足 `策略超额复合年化收益 > 0 AND (Excess Calmar Ratio > 1 OR 策略最大回撤同期超额 > 0)` 即通过。

...
- `10_qa_runner_outputs.sql` 新增 `QA-RESUME-1..5`，并新增 `p_rebalance_anchor_start`，用于分段 resume QA 复用原实验调仓周序。
- 新增 `sql/ml/strategy1/15_qa_ledger_resume_consistency.sql`，用于后续 full fresh vs resume segment 一致性验收。
...
- smoke 首日现金恢复为父状态现金 `135.9692847801162`，NAV 写出 11 个交易日、持仓 330 行、成交 36 行；本地报告使用 `--skip-gcs-upload`。
...
- BigQuery smoke：`08` resume、`09` resume、本地 `render_report.py --skip-gcs-upload`、`10_qa_runner_outputs.sql` 全部通过。
- `git diff --check`

...
- OQ-006 单位契约已实现并关闭：`ashare_meta.ods_field_unit_map` 已创建并填充 P0 + PR #13 首批映射；`dwd_index_eod.volume/amount` 已按 `vol*100` / `amount*1000` 修复换算并迁移为 `volume_share/amount_cny`；`sql/qa/05_oq006_unit_checks.sql` 已创建并纳入所有 DWD PR 必跑 QA；单位准入硬规则已写入 DWD-DIM §3.3-H 和 `KNOWN_CONSTRAINTS.md`。
- **下一步**：落地 PR #13 财务三表 DWD（`dwd_fin_income` / `dwd_fin_balancesheet` / `dwd_fin_cashflow`）并补全 `ods_field_unit_map` 剩余字段映射，跑通 `05_oq006_unit_checks.sql`；或并行提升 v0 模型质量与参数（OQ-010）、准备 GCS bucket（`ashare-artifacts`）+ ADC 后重跑 render。

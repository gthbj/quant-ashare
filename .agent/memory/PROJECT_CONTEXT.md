...
- 策略 1 风险特征入模与候选增强 PRD 已新增：`docs/prd/PRD_20260606_03_策略1风险特征入模与候选增强.md`。该 PRD 承接 PRD04 LightGBM binary / regression 均 rejected、尾部风险 P1/P2 交易层 profile 不足以建立默认策略的结果，定义下一轮 OQ-010 不默认启用 `skip_new_buys`，而是新增 `feature_set_id=strategy1_pv_fin_risk_v0_20260606`，将个股尾部风险、`dws_market_state_daily` 市场状态和风险 flag 纳入 Cloud Run frozen matrix；P0 固定 `diagnostic_only`，默认 40 候选 / 20 并发 / 2 vCPU 8Gi，复用 LightGBM binary + regression 和共享 acceptance contract；P1 再评估 `risk_score_penalty_v0` 候选风险评分。本文只写 PRD，未改代码、未运行 BigQuery / Cloud Run。
- OQ-005 GCP 数据流水线长期编排方向已在 2026-06-08 调整：`docs/prd/PRD_20260608_01_OQ005调度完全迁出Composer.md` 已明确用 `Cloud Scheduler + Cloud Workflows` 取代“长期保留 Cloud Composer”的方案。Cloud Run Jobs 继续负责 Tushare 兼容 API→GCS Parquet 采集，BigQuery SQL / Dataform 继续负责 ODS→DIM/DWD/DWS/ADS，简单告警检查迁到无 Composer 的定时入口；当前已落地的 Composer DAG 拆分、window refresh、alert checker、000001.SH 指数补数与 market-state 日更仍是 cutover 前过渡态，目标是在迁移验收后删除 Composer 环境。
...
- **下一步**：OQ-005 优先按 `docs/prd/PRD_20260608_01_OQ005调度完全迁出Composer.md` 推进编排层迁移：先落 `Cloud Scheduler + Cloud Workflows` 基础设施、手工 smoke `qa_only` / `daily_current` / `backfill` / 非交易日 skip，再完成生产 cutover 验收并删除 Composer 环境。OQ-010 继续保留为策略侧开放主线，但不要让策略实验阻塞 Composer 迁出这条基础设施成本优化路径。P1 数据扩展再做三大报表单季 `q_*` 派生和行业/资金/事件特征扩展。

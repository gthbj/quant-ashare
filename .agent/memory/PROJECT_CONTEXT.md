...
- OQ-010 首轮实验 runner 实现已在 PR #37 完成：新增实验 manifest 和对比报告脚本；`sql/ml/strategy1/01-06/09-12` 支持实验身份、调仓频率、目标持股/权重、`p_label_horizon` 和 `feature_set_id`；诊断脚本与 QA 已改为 horizon-aware；已通过 JSON/Python/`git diff --check` 和 BigQuery dry-run，尚未合并或端到端实跑实验。

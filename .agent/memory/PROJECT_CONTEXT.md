...
- 策略 1 尾部风险控制 PRD 已合并：`docs/prd/PRD_20260606_01_策略1尾部风险控制.md`。P0 固定最大回撤诊断已在分支 `codex/implement-tail-risk-diagnostics-p0` / PR #87 实现为 ADS/DWD/DIM 只读 artifact 链路：`scripts/strategy1/analyze_tail_risk.py` 输出 `tail_risk/` 最大回撤事件、持仓贡献、跌停/不可卖暴露、选股画像和 ADS pre/post hash；Cloud Run `backtest_report.py` 默认执行该诊断并跑 `sql/ml/strategy1/20_qa_tail_risk_outputs.sql`；TopK comparison 报告增加尾部风险摘要。P1 个股硬风险过滤 profile 已在分支 `codex/implement-tail-risk-p1` 实现到 PR 待合并阶段：默认 `diagnostic_only` 不改变选股，`individual_risk_guard_v0` 在 candidate selection 层写 `tail_risk:*` 风险标记但不把风险标记股票从 target 排除，Ledger v1 执行层只拦截未持仓新买入并写 `BUY_SKIPPED_TAIL_RISK`，避免强制卖出已有持仓；合并部署后仍需跑 full-period A/B。P2 市场状态 risk-off 风控仍待后续实现。
...
- **下一步**：OQ-010 先合并尾部风险 P1 profile 实现并部署新 Cloud Run runner 镜像，再执行 full-period `diagnostic_only` vs `individual_risk_guard_v0` A/B，比较收益、最大回撤、跌停/不可卖暴露、换手和 acceptance 状态；若 P1 不能改善回撤，再进入 P2 `dws_market_state_daily` / market risk-off 或下一模型族 / 特征增强。accepted Cloud Run Python baseline 建立后，再把 `docs/prd/PRD_20260604_02_策略1月度滚动重训.md` 从 BQML / SQL runner 口径改为 Cloud Run Python / backend-neutral 口径。OQ-005 继续补 Dataform、告警、补跑和完整 ODS→ADS 运维观测闭环。P1 数据扩展再做三大报表单季 `q_*` 派生和行业/资金/事件特征扩展。

...
- P0 DIM/DWD 建表 SQL 已落地到 `sql/`，但尚未实际物化到 BigQuery。
- 2026-05-31 ODS 已补采 `index_member_all` 与 `ci_index_member`，可落地申万/中信个股行业时点映射维表；OQ-001 已关闭。
- **下一步**：执行 P0 DIM/DWD 建表脚本并做 QA；随后落地 P0 DWS/ADS SQL（universe、价格/估值/财务特征、市场状态、标签、训练面板、预测/候选/组合/回测表）和 `ml_ranker_v0` 基线回测。

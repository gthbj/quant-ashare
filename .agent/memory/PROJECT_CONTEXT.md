...
- OQ-010 首轮实验 runner 实现已在 `codex/implement-oq010-experiment-runner` PR 分支完成：新增实验 manifest 和对比报告脚本；`sql/ml/strategy1/01-06/09-12` 支持实验身份、调仓频率、目标持股/权重、`p_label_horizon` 和 `feature_set_id`；诊断脚本与 QA 已改为 horizon-aware；已通过 JSON/Python/`git diff --check` 和 BigQuery dry-run，尚未合并或端到端实跑实验。
- **下一步**：review/合并 OQ-010 实现 PR 后执行第一轮对照实验。也可补 `dws_market_state_daily`。P1 再做三大报表单季 `q_*` 派生和行业/资金/事件特征扩展。

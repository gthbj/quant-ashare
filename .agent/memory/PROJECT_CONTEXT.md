...
- **下一步**：OQ-010 先合并并部署尾部风险 P0 诊断实现，随后用 Wave 3 Top5 统一生成 `tail_risk/` 证据，再基于该证据做 P1 个股硬风险过滤 profile A/B。accepted Cloud Run Python baseline 建立后，再把 `docs/prd/PRD_20260604_02_策略1月度滚动重训.md` 从 BQML / SQL runner 口径改为 Cloud Run Python / backend-neutral 口径。OQ-005 继续补 Dataform、告警、补跑和完整运维观测闭环；也可补 `dws_market_state_daily`。P1 再做三大报表单季 `q_*` 派生和行业/资金/事件特征扩展。

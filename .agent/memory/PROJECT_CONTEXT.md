...
- 策略 1 Cloud Run Python 模型基线搜索 PRD 已新增并进入实现 PR：`docs/prd/PRD_20260605_04_策略1CloudRunPython模型基线搜索.md`。该 PRD 是 sklearn LogisticRegression 首轮失败后的下一轮 OQ-010 搜索方案：固定数据截止 `2026-04-30`，train/valid/test/final_holdout 为 `2019-04-03..2023-12-31` / 2024 / 2025 / `2026-01-05..2026-04-30`；固定 `pv_fin_quality + 30/5% + biweekly + 5d`、沪深主板股票池、成本 profile 和 Ledger v1；P0 推荐 LightGBM wave 2，使用 2021/2022/2023 purged walk-forward CV + 2024 valid confirmation 选 Top 5，2025 test 做硬接受门，2026 final holdout 只做明显坏结果 veto / holdout watch，并要求实现共享验收配置 `configs/strategy1/model_acceptance_contract_v1.yml`。实现分支 `codex/implement-prd04-cloudrun-python-baseline` 已完成 PR #79 review follow-up：共享契约注入 `18`/`19` QA、final_holdout 缺失转 `needs_more_evidence`、auto-next-wave QA 后触发、NULL-safe QA 和 unmatched 兜底均已修复；真实 wave 2 search `cloudrun_python_lgbm_pvfq_n30_bw_h5_20260605_01` 已完成，Top5 全部 `rejected`，不建立 `cloud_run_python_baseline_v1`。LightGBM candidate task 资源口径已从原设计 1 vCPU / 4Gi 收敛为 2 CPU / 8Gi，Job spec `parallelism=20`，40 个候选仍一次 execution 启动。
...
- **下一步**：OQ-010 优先合并 PRD04 实现 PR #79，构建/部署包含 review follow-up 的新 Cloud Run 镜像，然后执行 wave 3 `lightgbm_regression` search。accepted baseline 建立后，再把 `docs/prd/PRD_20260604_02_策略1月度滚动重训.md` 从 BQML / SQL runner 口径改为 Cloud Run Python / backend-neutral 口径。OQ-005 继续补 Dataform、告警、补跑和完整运维观测闭环；也可补 `dws_market_state_daily`。P1 再做三大报表单季 `q_*` 派生和行业/资金/事件特征扩展。

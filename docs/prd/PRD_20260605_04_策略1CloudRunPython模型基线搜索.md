...
2. 2026 年 5 月行情 / 特征虽然已进入 ODS/DWD/DWS，但 5d / 10d / 20d 标签在当前日期仍不完整。
3. 为避免把不完整标签或最新观察期反复纳入调参，本轮不使用 2026 年 5 月数据。
...
4. 使用 valid-only 规则筛选 shortlist，避免用 test / final holdout 直接选参数。
...
Wave 2 候选数建议控制在 40-80 个，不做完整笛卡尔积。候选 manifest 应人工分层覆盖以下维度：
...
若 Wave 2 没有 accepted 候选，P1 进入 Wave 3。Wave 3 可选两条路线：

| 优先级 | 模型族 | 说明 |
|---|---|---|
| P1-A | `xgboost_hist` | 更强的树模型对照，需引入 `xgboost` 依赖 |
| P1-B | `sklearn_hist_gradient_boosting` | 依赖更轻，但能力通常弱于 LightGBM / XGBoost |
...
候选排序只使用 valid split。主排序规则：

1. `valid_rank_ic_mean` 降序。
2. `valid_top_minus_bottom_fwd_ret_mean` 降序。
3. `valid_icir` 降序。
4. `valid_auc` 降序。
5. 在前四项接近时，优先更低 turnover proxy 和更简单模型。

进入 Top 5 的最低门槛：

1. `valid_rank_ic_mean > 0`。
2. `valid_top_minus_bottom_fwd_ret_mean > 0`。
3. `valid_signal_status='stable'`。
4. candidate 训练状态为 `succeeded`。
5. 模型收敛 / early stopping 状态不得为失败。
...
| `needs_more_evidence` | valid/test 通过但 final holdout 或稳定性证据不足，需要 owner 决策或后续数据 |
| `rejected` | 明确不接受 |
| `failed` | 执行失败，不参与模型结论 |
...
1. valid: `valid_rank_ic_mean > 0`。
2. valid: `valid_top_minus_bottom_fwd_ret_mean > 0`。
3. valid: `valid_signal_status='stable'`。
4. test: `test_rank_ic_mean > 0`。
5. test: `test_top_minus_bottom_fwd_ret_mean > 0`。
6. test: `test_year_excess_return_vs_000852 > 0`。
7. final_holdout: `final_holdout_excess_return_vs_000852 >= 0`。
8. overall_report: `overall_excess_return_vs_000852 > 0`。
9. 回测 summary: `total_return > 0`。
10. 回测 summary: `max_drawdown` 不得显著劣于历史 BQML reference 和主基准；具体阈值由 QA 配置化，P0 默认不得低于 `-20%`。
11. 诊断结论不得为 `signal_inverted`、`unusable_signal`、`sample_filter_risk high` 或等价失败状态。
12. 所有 runner / diagnosis / search QA 通过。

### 9.3 Needs-more-evidence

若候选满足 valid/test 门槛，但 final_holdout 出现轻微跑输：

```text
-5pct < final_holdout_excess_return_vs_000852 < 0
```

则状态必须为 `needs_more_evidence`，不得自动登记为 baseline。owner 可以选择：

1. 等待更多 2026 数据形成新的 holdout。
2. 允许作为临时 research baseline，但不能登记为 production baseline。
3. 进入下一模型族或特征增强波次。
...
1. test 年度超额收益 `<= 0`。
2. final_holdout 跑输中证1000超过 5pct。
3. final_holdout 总收益低于 `-8%`。
4. valid/test 任一 RankIC 为负。
5. valid/test top-minus-bottom 同时为负。
6. 诊断识别为反向信号或不可用信号。
7. QA 失败。
...
4. 2026 final_holdout 不参与 candidate 排名，只参与接受 / 风险复核。
...
1. 容器镜像加入 `lightgbm` 依赖。
2. 扩展 candidate manifest schema，支持 `model_family=lightgbm_gbdt`。
3. 扩展 `train_candidate_fanout`，支持 LightGBM 训练、valid 预测和 metrics。
4. 扩展 reducer / ranking / Top5 完整回测流程。
5. 增加 search comparison artifact 和 baseline acceptance 写入。
6. 增加 `19_qa_cloudrun_python_baseline_search_outputs.sql`。
...
2. 一次 prepare matrix。
3. 40-80 个 LightGBM candidates task fan-out。
4. valid-only 选 Top 5。
5. Top 5 完整回测到 `2026-04-30`。
6. 跑 `10` / `12` / `19` QA。
7. 输出是否建立 `cloud_run_python_baseline_v1`。
...
3. 若 rejected：进入 Wave 3 或先补 `dws_market_state_daily` 后再搜索。

## 14. 风险与控制

| 风险 | 控制 |
|---|---|
| 反复看 test / final holdout 导致过拟合 | valid-only 排名；记录 `test_reuse_wave_no`；超过 3 波必须新增 holdout |
...
| final_holdout 过短导致误判 | final_holdout 只做风险复核；轻微跑输归为 `needs_more_evidence` |
...
5. PRD 明确 2026 final_holdout 不参与参数排名，只参与接受门和风险复核。
6. PRD 明确 accepted / needs_more_evidence / rejected 机器化标准。
7. PRD 明确新增 QA 和产物契约。
8. TODO / 记忆同步到新的 OQ-010 下一步。

...
若 `candidate_count > 40`，P0 必须启用 §8.2 的 purged walk-forward CV 排名确认，不能只用单一年份 2024 valid 直接从高容量候选中选 Top 5。若实现阶段暂不支持 CV，则本轮候选数必须压到 `<=40`，并在 comparison artifact 中标记 `selection_variance_risk`；没有 CV 证据的 `candidate_count > 40` search 不得登记 `accepted` baseline。
...
2. Wave 3 或后续修订可加入 `lightgbm_lambdarank`、`lightgbm_regression` 或 XGBoost ranking / regression 作为候选模型族。
3. 引入 ranking objective 前，必须先定义按 `trade_date` 分组的训练数据契约、eval group 契约、score orientation 契约和对应 QA。
...
| `cv_2024` | `2019-04-03` 至 `2023-12-31` | `2024-01-02` 至 `2024-12-31` |
...
1. `cv_confirmation_status='passed'`；若 `candidate_count<=40` 且实现暂不启用 CV，可为 `not_required`，但必须记录原因。
...
| 领域 | 字段 | 门槛 |
|---|---|---|
| CV | `cv_confirmation_status` | `passed`；若 `candidate_count<=40` 且未启用 CV，可为 `not_required` |
...
1. `candidate_count > 40` 但没有 `cv_confirmation_status='passed'`。
...
| `cv_confirmation_status` | `passed` / `not_required` / `missing` |
...
14. `candidate_count > 40` 时，`accepted` 候选必须有 `cv_confirmation_status='passed'`。
...
2. 容器镜像加入 `lightgbm` 依赖。
3. 扩展 candidate manifest schema，支持 `model_family=lightgbm_gbdt`。
4. 扩展 `train_candidate_fanout`，支持 LightGBM 训练、CV / valid 预测和 metrics。
5. 扩展 reducer / ranking / Top5 完整回测流程。
6. 增加 search comparison artifact 和 baseline acceptance 写入。
7. 增加 `19_qa_cloudrun_python_baseline_search_outputs.sql`。
...
4. CV + 2024 valid confirmation 选 Top 5。
...
| 单一年份 valid 从高容量候选中选参方差过高 | `candidate_count > 40` 必须启用 purged walk-forward CV；未启用则不得 accepted |
...
| 二分类训练目标与排序评估指标不完全一致 | P0 先保留 binary 兼容当前链路；后续以 ranking / regression objective 作为新波次 |
...
8. PRD 明确新增共享验收契约、QA 和产物契约。
9. TODO / 记忆同步到新的 OQ-010 下一步。

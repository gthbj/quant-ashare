...
| DWS 价格特征 | 2015 起有行，但 **`2015-Q1` 全部 150,726 行 `has_full_history_60d=FALSE`**（backfill 时 DWD 无 2014 lookback buffer）；**`2019-Q1` 全部 208,007 行也是 FALSE**——陈旧标记：旗标在 2019-01-01 起首次物化时计算，2015-2018 backfill 完成后没有回刷 2019Q1 |
...
关键事实：**2019Q1 旗标修复不需要任何新数据**（DWD 已有 2018 行，重刷窗口即可）；2015Q1 修复只需
2014 lookback（ODS 已有）；而 ODS 2010 起可用使整个历史下限可以一次性前移，永久消除边界缺口问题。

## 2. 目标

1. **Phase A 数据层**：DWD/DWS 历史下限前移（推荐 2010），所有边界 `has_full_history_60d` 旗标按真实
   lookback 重算，2019Q1 陈旧旗标修复；估值/财务字段逐年完备度报告。
...
2. **2015Q1 旗标修复**：2014 DWD 行就位后重刷 2015 年初窗口，`has_full_history_60d` 按真实 60 日
   lookback 重算。
3. **2019Q1 陈旧旗标修复**：重刷 2019Q1 窗口（lookback 读已有 2018 DWD 行），无需新数据。该项可最先做、
   独立验证。
...
- 每年跑 `qa_refit_register_predict_outputs`，并新增断言：`effective_refit_train_start ==
  nominal_refit_train_start`（不再允许 clamp）；覆盖检查含**窗口内部缺口**（吸取 2019Q1 端点检查
  漏内部洞的教训），按开市日逐日 left join 断言 panel 覆盖。
...
| 旗标正确性 | 2019Q1 与 2015Q1（及新下限边界外的所有年份）`has_full_history_60d` 按真实 lookback 重算；只有数据宇宙真实边界（新下限后约一季度）允许 FALSE 集中出现 |
...
1. §4.1 盘点矩阵 → 定历史下限。
2. §4.2.3 2019Q1 旗标修复 + parity QA（最小独立闭环，先验证重刷通道）。
3. §4.2.1-2 2010-2014 backfill（按年）→ 2015Q1 旗标修复 → 完备度报告。

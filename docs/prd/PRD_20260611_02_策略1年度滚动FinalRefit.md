> 文档维护：Claude Fable 5；Post-implementation note / coverage revision：GPT-5 Codex（最近更新 2026-06-11）

# PRD：策略 1 年度滚动 Final Refit

> 状态：已实现；2026-06-11 coverage follow-up 修订。
...
5. `2021-2026` 六年从 select 之后重跑（dedicated refit panel + refit + predict + 可选年度 diagnostic），**不重跑 selection panel / matrix / 11 候选 fanout**。

## 3. 非目标

- 不改变 11 个 LightGBM regression 候选池和 valid 选参 / 排序规则。
- 不改 `biweekly`、20 只持仓、7.5% 单票上限等组合口径。
- 不重建 selection panel / matrix，不重跑候选 fanout；final refit 需要为 refit run 构建独立 panel。
...
覆盖修订（2026-06-11）：当前生产 DWS / sample 覆盖在 `2019-04-03` 之前存在不可训练缺口，主要包括历史 valuation 行稀疏与 `has_full_history_60d=FALSE` 的 2019Q1 区间。年度 final refit 的实际执行窗口因此采用：

```text
effective_refit_train_start = max(nominal_refit_train_start, DATE '2019-04-03')
effective_refit_train_end   = nominal_refit_train_end
```

当前 2021-2026 effective refit 窗口：

| 回测年 | effective refit 窗口 | 说明 |
|---|---|---|
| 2021 | 2019-04-03 ~ 2020-12-24 | 名义 2016-01-04 起点受当前 DWS 覆盖下限截断 |
| 2022 | 2019-04-03 ~ 2021-12-24 | 名义 2017-01-03 起点受当前 DWS 覆盖下限截断 |
| 2023 | 2019-04-03 ~ 2022-12-23 | 名义 2018-01-02 起点受当前 DWS 覆盖下限截断 |
| 2024 | 2019-04-03 ~ 2023-12-22 | 名义 2019-01-02 起点受当前 DWS 覆盖下限截断 |
| 2025 | 2020-01-02 ~ 2024-12-24 | 不受 2019-04-03 下限影响 |
| 2026 | 2021-01-04 ~ 2025-12-24 | 不受 2019-04-03 下限影响 |

该修订是当前数据覆盖下的可执行口径，不代表 2021-2024 已完成名义完整五年 refit。若后续 owner 要求真实 pre-2019 五年窗口，需要先修复 / 重建 DWS lookback 与历史 valuation 覆盖，再重跑 dedicated refit panel、refit 与 continuous ledger。

## 5. 核心设计

### 5.1 复用层级：DWS/SQL panel 构建逻辑可复用，selection panel 与冻结 matrix 不可复用
...
正确的复用层级是 **BigQuery DWS + panel SQL 构建逻辑**（raw_v0 特征），而不是已经按 selection split 写出的 selection panel 行：

- 每年 selection panel 的 run_id 归属 selection run，且包含 selection train / valid / test split 与 label-embargo 边界；live review follow-up 已实证它在 refit 窗口内会出现内部交易日缺口。
- annual plan 必须在 select 之后新增 `build_refit_training_panel` step，用 refit run_id 构建 dedicated refit panel；`source_panel_run_id` 指向 refit run_id。
- refit 必须从 dedicated refit panel 读取 effective refit 窗口的原始特征行，**重新 fit preprocessor**，再训练、再预测。
- 冻结 matrix 与其 `preprocess.joblib` 对 refit 一律只读不复用，继续作为 selection run 的 audit 产物。

实现 PR 必须包含前置断言：dedicated refit panel 对 effective refit train window 的每个 SSE 开市日都有 labeled train 行，对 prediction source window 的每个 SSE 开市日都有 panel 行；不满足时 fail-fast，禁止静默缩窗。

> Post-implementation note（2026-06-11）：live review follow-up 已推翻“selection panel 连续覆盖 refit 窗口”这个实现假设。审计确认 source selection panel 在 refit 训练窗口内存在内部交易日缺口：2021/2022/2023 均缺 `2019-01-02..2019-04-02`，且多个年份存在 selection split / label-embargo 造成的年末缺口。修订后的可执行方案是 dedicated refit panel + `2019-04-03` effective coverage floor。OQ-014 关闭前，不得把本轮 official continuous 结果当作名义完整五年 final-refit 窗口已实证覆盖的 accepted baseline。
...
- `source_panel_run_id`：该年 **refit run 的 run_id**。annual plan 必须先用 refit run_id 执行 `build_refit_training_panel`，再执行 `refit_register_predict`。读 panel 与写 registry / prediction / artifact 均使用 refit run_id；`source_run_id` 仍保留 selection run_id，用于读取 selected candidate lineage。
- resolved plan 的 effective `final_refit` 窗口。

动作：
1. 从 dedicated refit panel 读取 effective refit 窗口行，fit 新 preprocessor。
...
- 溯源字段（registry 列或 `metrics_json`）：`selected_candidate_id`、`source_run_id`（selection run）、`source_panel_run_id`（dedicated refit panel run，当前等于 refit run_id）、`refit_train_start/end`、`refit=true`。
...
**Annual orchestrator resolved plan**（`orchestrate_annual_rolling_selection`）：每年 `year_plan` 必须把 `final_refit` 从纯元数据块（`planned_after_candidate_selection`）升级为**可执行命令步骤**——`select` 之后先插入 `build_refit_training_panel` step，再插入 `final_refit` step，命令调用 `refit_register_predict` 入口（与 PR #159 给 plan 插入 panel step 同模式）；plan 的步骤顺序断言测试同步扩展。

**Pipeline scheduler DAG**（`PRD_20260611_01`）：在 select 与 diagnostic backtest 之间插入 refit panel 与 refit stage：

```text
select:yYYYY -> refit_panel:yYYYY -> refit:yYYYY -> diagnostic_backtest:yYYYY
```

- `refit_panel:yYYYY` 复用 SQL runner / `build_training_panel_risk_feature`，写 refit run_id 名下 panel。
...
1. refit model 的 `train_start_date/train_end_date` == resolved plan 的 effective `final_refit` 窗口（逐年）。
...
4. panel 覆盖断言按 `source_panel_run_id` 查询：该 run 名下 panel 行按 SSE 开市日完整覆盖 effective refit train window 与 prediction source window（§5.1 的前置断言同口径）。
...
6. refit registry 行的 `source_panel_run_id` 非空，且等于该年 refit run_id（与 resolved plan 对账）。

## 6. 重跑范围

- 六年 `build_refit_training_panel`（6 个 dedicated refit panels）。
- 六年 `refit_register_predict`（6 个单模型训练 + 预测）。
- 可选：六年年度 diagnostic backtest 用 refit prediction 重跑（仍是 diagnostic，不是正式结果）。
- 不重跑：selection panel、matrix、11 候选 fanout、select（选参结论不变——valid 选参只依赖已有 fanout 结果）。

## 7. 验收标准

| 验收项 | 要求 |
|---|---|
| refit 窗口 | 六年 refit model 训练窗口逐年等于 resolved plan effective `final_refit` 窗口，QA 断言通过 |
| 数据复用 | 全程未新建 matrix / 未重跑候选；refit 从 dedicated refit panel 读取，不消费 selection panel 或冻结 matrix 的 transformed arrays |
...
| panel 覆盖不足 refit 窗口 | 前置断言 fail-fast；唯一允许的缩窗是 resolved plan 中显式记录的 `2019-04-03` effective coverage floor |
...
1. 实现 `build_refit_training_panel` plan step 与 `refit_register_predict` 步骤 + 单元测试（窗口断言、mask 构造、registry 契约）。
2. scheduler DAG / catalog / runbook 同步，确保 `select -> refit_panel -> refit`。

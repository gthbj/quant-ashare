...
| 项 | 默认口径 |
|---|---|
| 标的 | A 股普通股票，含退市历史；首个基线仅沪深主板，不含北交所、创业板、科创板 |
| 频率 | 日线 EOD |
| 信号时点 | `t` 日收盘后 |
...
|---|---|---|
| 主板 | 开 | 基础股票池 |
| 创业板/科创板 | 关 | 不纳入首个基线；后续可单独做对照实验 |
| 北交所 | 关 | 不纳入首个基线；流动性和制度差异大，建议单独回测 |
| ST | 关 | P0 剔除 |
| 次新股 | 关 | 默认上市 60 日内剔除 |
...
1. P0 回测默认成本参数已由 `PRD_20260602_02_OQ010交易成本口径.md` 固化：佣金万一免五、卖出印花税 5 bps、买/卖滑点各 5 bps；仍需后续 runner 实现。
2. P0 默认调仓频率：周度还是日度滚动。
3. P0 默认持股数和单票上限。
4. 首个基线板块纳入口径已定：仅沪深主板，不含北交所、创业板、科创板；后续如需纳入，作为独立对照实验或单独模型。
5. 首个模型训练工具链已定为 BigQuery ML + SQL runner；后续待确认的是调仓、持股数和单票权重上限。

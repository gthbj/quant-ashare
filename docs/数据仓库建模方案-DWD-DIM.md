...
> 文档维护：Claude Opus 4.8（最近更新 2026-05-31）；§4.6 回填范围与 ODS 清单更新：GPT-5（2026-05-31）
...
| 表 | 源 | 用途 |
|---|---|---|
| `dwd_index_eod` | `index_daily`(+`index_dailybasic`) | 基准指数日线与估值：`sec_code` 输出 canonical 指数代码（如沪深300 `000300.SH`），`source_sec_code` 保留 ODS 实际代码（如 `399300.SZ`），映射先用脚本 CTE 维护，未来可沉淀为 `dim_index`。用于超额收益标签、市场状态、beta。 |
...
4. **基准指数代码可用性**：§6.4 列出的中证1000/中证2000/国证2000 等代码需先核对 ODS 实际 `ts_code` 端点，再映射到 DWD canonical `sec_code`；双代码或多代码指数需维护 `source_sec_code -> sec_code` 映射，避免 DWS/ADS 直接依赖 ODS 代码。

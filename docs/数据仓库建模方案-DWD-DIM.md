...
- **FR-UNIT-1：标准字段必须带单位后缀**。金额字段统一 `_cny`，数量/股本字段统一 `_share`，保留源单位字段时用 `_k_cny`、`_10k_cny`、`_lot`、`_10k_share` 等后缀。示例：`daily.amount` → `amount_k_cny`（raw，千元）；`daily.amount * 1000` → `amount_cny`（standard，元）。例外：`source_name_passthrough` 适用于 `source_unit = canonical_unit` 且 `multiplier = 1` 的 pass-through 字段（如 Tushare 财报金额字段已为元，无需换算），允许保留源字段名，不受「新 DWD 表不得新增未带单位后缀」的限制；`legacy_unsuffixed` 仅用于历史已有字段，必须登记 `naming_exception_expires_at` 清理日期。
...
- **FR-UNIT-5：新 DWD PR 必须附单位映射**。新增或修改 DWD 标准字段时，PR 必须同时新增或更新 `ashare_meta.ods_field_unit_map` 记录和 `sql/qa/05_unit_contract_checks.sql` 覆盖范围，并把 `05_unit_contract_checks.sql` 纳入该 PR 的必跑 QA。没有 `verified` 映射，不得合并。

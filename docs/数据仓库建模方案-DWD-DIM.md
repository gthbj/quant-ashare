...
2. **ODS 现状**：`ashare_ods` 下共 56 张外部表，全部以 `partition_date`（`STRING`，`YYYYMMDD`）+ `endpoint` 作为 Hive 分区键。**任何查询都必须带 `partition_date`/`endpoint` 过滤**，否则 BigQuery 直接报错（强制分区裁剪）。
...
|---|---|---|---|---|
| daily / adj_factor / daily_basic / stk_limit / suspend_d / moneyflow … | `partition_date==trade_date` | trade_date | trade_date | partition_date |
...
> **事件表 DWD 不继承 B 类统一模板**：每张表单独定义可见时间与回填窗口。完整 56 表矩阵在建表前逐表补全。
...
### 5.6 `dim_index_weight` —— 指数成分权重（P1，缓变维）
...
- **构建要点（核心 SQL 骨架）**：
  ...
  d AS (                              -- ODS daily：出口归一 sec_code + 单位换算
    SELECT ts_code AS sec_code, PARSE_DATE('%Y%m%d', trade_date) trade_date,
           ...
           vol*100 AS volume, amount*1000 AS amount,          -- 手→股、千元→元
  ...
  SELECT
    ...
    d.volume, d.amount, adj.adj_factor,
...
SELECT a.sec_code, a.seq AS t_seq,
       ...
       (b.open_hfq IS NOT NULL AND c.close_hfq IS NOT NULL) AS label_valid
FROM px a
JOIN px b ON b.sec_code=a.sec_code AND b.seq=a.seq+1               -- t+1（市场交易日）
JOIN px c ON c.sec_code=a.sec_code AND c.seq=a.seq+5              -- t+5
```
> 关键：**入场价用 `t+1`**，`t±k` 走**市场交易日序列**；多周期标签（`fwd_ret_1d/5d/10d/20d`）并存；`entry_reachable`/`exit_reachable`/`label_valid` 标记可成交性，训练/回测据此过滤或加权，使标签贴近真实可成交收益。
...
- **可见日字段按表存在性而定**（实测，56 张表）：`income`/`balancesheet`/`cashflow` 有 `f_ann_date`；`fina_indicator` **无 `f_ann_date`**，只能用 `ann_date`。

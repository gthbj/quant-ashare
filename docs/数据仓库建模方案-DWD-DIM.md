...
> 下表是 54 张 ODS 表 → 目标 DWD/DIM 的完整映射。`类`列对应 §0 的三类分区语义（A 行情增量 / B 财务公告 / C 维度快照）。
...
> **事件表 DWD 不继承 B 类统一模板**：每张表单独定义可见时间与回填窗口。完整 54 表矩阵在建表前逐表补全。
...
- **可见日字段按表存在性而定**（实测，54 张表）：`income`/`balancesheet`/`cashflow` 有 `f_ann_date`；`fina_indicator` **无 `f_ann_date`**，只能用 `ann_date`。

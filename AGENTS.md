...
例外：纯只读盘点 / 问答 / 评审，且未改项目文件、未产生 owner 已采纳的状态变化或新决策时，不追加 handoff、不更新状态/TODO；评审按「六、评审协议」优先写 GitHub PR comment，只有 owner 明确要求或无 PR comment 承载面时才写评审文档。
...
- **GitHub PR review 默认写 PR comment**：如果评审对象在 GitHub PR 中，评审结论直接写到 PR comment；一条 comment 写不下就拆成多条。不要额外创建 `docs/reviews/` 文档，除非 owner 明确要求归档成文档。
- **无 PR comment 承载面或 owner 明确要求文档时**，才产出评审文档，放 `docs/reviews/`，命名 `<对象>-review[-<专题>].md`。文档至少含：评审对象（PR / commit / 文件）、按严重度分级的发现、每条的依据 / 影响 / 建议、与既有决策是否冲突、结论；并带模型署名（§五）。
- **只写发现的问题**：验证通过的项、做对的地方、已确认一致的决策核对等**不写入评审文档或 PR comment 的发现清单**。评审产物的价值是可追溯的问题清单，不是工作量证明。

...
关键点：
- 当前定时入口 `ashare_pipeline_alert_checker` 最多每小时执行一次
- checker 查询窗口为 70 分钟，和 60 分钟调度形成重叠，避免调度延迟时漏报
...
# 按生产窗口查询最近 70 分钟异常
python scripts/alerting/check_alerts.py --lookback-minutes 70

# 写入 Cloud Logging（供告警链路使用）
python scripts/alerting/check_alerts.py --write-log --write-heartbeat --lookback-minutes 70

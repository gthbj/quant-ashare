...
- [ ] OQ-010 / 工程治理：迁移五个 Strategy1 Cloud Run Job command 到 package entrypoint
  说明：需在 package entrypoint 代码 PR 合并后，单独从 main 构建镜像，并一次性覆盖四类 cutover 面：1）五个正式 Cloud Run jobs 的 command args 从 `scripts.strategy1_cloudrun.*` 切到 `quant_ashare.strategy1.*`；2）orchestrator / pipeline-control / native search / annual rolling 等 Cloud Run override args 中硬编码的 module 字符串同步切换；3）`configs/strategy1/active_step_catalog.yml` 的 `caller` 字段同步切换；4）active runbook / 示例命令同步切换。完成后对五个 jobs 跑 `--help` boot smoke，并清理 PR #153 验证留下的临时 job `strategy1-package-entrypoint-help-smoke`；删除旧 wrapper 的前置条件是 active scopes 内 grep 不到五个旧模块路径，并把旧模块路径纳入 retired-reference linter 防回流。

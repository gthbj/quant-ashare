...
- [ ] OQ-010：按 `PRD_20260611_10` 跑自上而下整手构造 Phase 0 / Phase 2 验证
  说明：Phase 1 代码准备已在分支 `codex/topdown-lot-construction` 实现：新增显式 opt-in `ledger_exec_v2_lot100_topdown`、full-rank `stock_candidate_daily.rank_raw <= walk_depth` 输入、自上而下整手买入、P1 profile 绑定替换语义、零现金缩股 QA、实际持仓数与最大单票权重 metrics/report/runbook。剩余工作是先跑 Phase 0 paper 双臂原型（带/不带 P1），再在合并/部署后跑 Phase 2 research-only continuous 重跑与三方对比交 owner；不动 v1、不改全局默认 profile、不 promotion。

...
- [ ] OQ-010：按 `PRD_20260611_10` 实现自上而下整手组合构造
  说明：Phase 0 paper 双臂原型已完成（报告 `docs/分析-策略1自上而下整手组合Phase0-20260612.md`，CSV 为脚本可再生成的本地产物，不随 PR 提交）。主结果 `walk_depth=50` / `20bps`：T0 CAGR=`10.22%`、MaxDD=`-63.02%`、Calmar=`0.162`；T1 CAGR=`-4.57%`、MaxDD=`-68.14%`、Calmar=`-0.067`，但 crunch excess 比 T0 改善 `+9.93pp`。预登记触发三项复核：P1 替换成本假设被证伪（T1-T0 CAGR `-14.80pp`）、双臂 MaxDD 均比 official 深超 15pp、T1 `max_realized_weight=100%`。Phase 1 `ledger_exec_v2_lot100_topdown` 代码准备是否继续、P1 绑定/无上限是否调整，需 owner 看过 Phase 0 后决定；仍不动 v1、不改默认 profile、不 promotion。

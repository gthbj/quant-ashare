...
- [ ] OQ-014：决定是否接受 effective-window annual final refit 结果进入 baseline 评估
  说明：2026-06-11 review follow-up 已审计确认，source selection panel 在 refit 训练窗口内存在内部缺口；PR #173 已完成工程缓解并实跑通过：annual plan 在 select 后构建 dedicated refit panel，`source_panel_run_id` 指向 refit run，effective refit start 取 `max(nominal_start, 2019-04-03)`，2021-2026 dedicated panel / refit / official continuous QA 均通过。剩余不是工程执行问题，而是 owner 方法论决策：接受当前 DWS 覆盖下的 effective-window 结果继续评估，或先修复 / 重建 DWS lookback 与历史 valuation 覆盖后追求 true pre-2019 五年窗口。

- [ ] OQ-010：基于 official continuous 结果决定下一轮策略改进或 accepted baseline 路线
  说明：2021-2026 official continuous 只产出数据与 QA 事实，尚未宣称 accepted baseline。最新 effective-window official continuous：compound_annual_return=`0.12036528993503204`，max_drawdown=`-0.4548151193656952`，information_ratio=`0.5420201365046585`。若继续推进，需先处理 OQ-014 的口径选择，再围绕特征、候选空间、风控或 acceptance gate 做独立方案；不得把本轮结果直接 promotion 或标 accepted。

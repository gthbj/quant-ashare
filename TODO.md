...
- [ ] 按 `docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md` 实现 sklearn native 模型实验：固定 `pv_fin_quality + 30/5% + biweekly + 5d` 交易口径和 `2019-04-03` 至 `2023-12-31` 训练窗口，一次 `prepare_matrix` 后并发训练 36 个 LogisticRegression 候选，只按 valid 指标选 Top 5，再对 Top 5 跑完整预测/组合/回测/报告/诊断和 `18` QA；accepted 候选必须 `valid_signal_status=stable`，跨模型族复用 2025 test 必须记录波次和 owner 批准，最终决定是否接受 `cloud_run_sklearn_native_baseline_v1`
...
- [~] OQ-005 GCP 数据流水线落地：`docs/prd/PRD_20260603_03_GCP数据流水线方案.md` 与 `docs/prd/PRD_20260605_01_OQ005剩余调度链路.md` 已定义 Cloud Run Jobs 采集、Dataform / BigQuery Studio pipeline 做 ODS→ADS、Cloud Composer 编排；Phase 0 已实现采集 manifest、14 张 schema contract、meta 表 DDL 与采集脚本 stub；Phase 1/1.5/1.6 已实现 endpoint worker、GCS staging/publish、小范围写入 smoke、ODS 可读性 QA、Composer 3 环境和每日轻量 readiness DAG；Phase 1.7 已新增 `ashare-ingest-current-scope` 单 execution 生产入口、Direct VPC egress + Cloud NAT 固定出口，修复 Composer scheduler queued 问题为 default Celery queue 路径，并完成 `2026-05-20` 至 `2026-06-03` SSE 开市日生产 GCS 回填和 `2026-06-04` Composer 生产 DAG 首跑。Phase 2.0 BigQuery SQL 兼容路径已由 PR #61 进入 `main`。Phase 2.2 股票 DWD/DWS 窗口刷新已在分支 `codex/windowed-dwd-dws-refresh` 实现并跟进 PR #65 review：新增 `sql/incremental/01_refresh_stock_dwd_dws_window.sql`、`sql/qa/10_windowed_stock_refresh_checks.sql`、`scripts/qa/run_windowed_refresh_equivalence.py`；DAG 的 `daily_current/backfill` 真实写入分支会刷新 DIM 小表、恢复 P0 metadata、按窗口 DELETE/INSERT 股票 DWD 与策略 1 DWS，并跳过 ADS runner/backtest 产物；窗口 DML 已加 transaction 和目标表存在性守卫，等价 QA runner 可定期把 canonical full SQL 与 window SQL 输出做 shadow 表数值对比。PR #65 合并后部署前真实 scratch QA 已发现并修复两个问题：QA runner 复制 `_full` 表时需带分区过滤，估值特征需按每只股票实际 60 条估值观测推导读取边界并校验 `build_start_date` 足够早；修复后 9 张目标表 full-vs-window mismatch 均为 0。下一步部署到 Composer 后做 `skip_ingestion=true` + `warehouse_mode=backfill` 窗口 smoke、`daily_current` scheduler smoke、`qa_only` 验收和 `full_rebuild_compat` 维护链路 smoke；之后进入 Dataform definitions、策略 runner/report 可选分支和告警/补跑/状态闭环

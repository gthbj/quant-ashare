...
- [~] 按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 与 `docs/prd/PRD_20260605_02_策略1CloudRun轻量Task并发.md` 实现策略 1 Cloud Run 训练回测执行器：首版 Cloud Run runner 与 orchestrator 状态/锁增强已由 PR #56/#57 合并，真实 Cloud Run smoke 已跑通并通过 `16`/`17` QA（smoke 模式允许 parity 未通过）。2026-06-05 task fan-out P0 已完成代码实现、部署和低成本真实 smoke：新增/更新 `prepare_matrix`、candidate task、reducer、orchestrator `--train-mode task_fanout` / `--candidate-parallelism`、BigQuery job labels/audit、`16`/`17` task fan-out QA 和运行手册；默认 5 个候选 smoke 已跑通 `cloudrun_prepare_matrix` / 5 个 candidate task / `cloudrun_select_register_predict` / `cloudrun_backtest_report`，`10`/`12`/`16`/`17` QA 全部通过。正式替代 BQML 前仍需 sklearn parity passed 或 owner 接受新 Cloud Run baseline，并补 Python ledger vs SQL ledger 正式等价验收

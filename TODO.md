...
- [~] 按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 实现策略 1 Cloud Run 训练回测执行器：首版 Cloud Run runner 已由 PR #56 合并，包含 sklearn logistic 训练/预测、Python fresh-start `ledger_exec_v1`、Cloud Run orchestrator、Docker/Cloud Build 配置、运行手册、meta 扩展 SQL 和 `16_qa_cloudrun_runner_outputs.sql`；当前分支 `codex/cloudrun-orchestrator-state-lock` 补 orchestrator 状态表写入、GCS generation-guarded lock、`--resume` / `--resume-from-step` 和 `17_qa_cloudrun_orchestrator_status.sql`，已通过本地 dry-run / py_compile / BigQuery dry-run / `git diff --check`；待 PR review/merge 后做真实 Cloud Run/BQ smoke、sklearn vs BQML parity 验收、Python ledger vs SQL ledger 等价验收

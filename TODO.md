...
- [~] 按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 与 `docs/prd/PRD_20260605_02_策略1CloudRun轻量Task并发.md` 实现策略 1 Cloud Run 训练回测执行器：首版 Cloud Run runner 与 orchestrator 状态/锁增强已由 PR #56/#57 合并，真实 Cloud Run smoke 已跑通并通过 `16`/`17` QA（smoke 模式允许 parity 未通过）。2026-06-05 已在分支 `codex/cloudrun-task-fanout` 实现 task fan-out P0：新增 `prepare_matrix` frozen matrix、candidate task、reducer、orchestrator `--train-mode task_fanout` / `--candidate-parallelism`、BigQuery job labels/audit、`16`/`17` task fan-out QA 和运行手册；已通过 Python py_compile、入口 dry-run、orchestrator `--tasks` 计划 dry-run、`16`/`17` BigQuery dry-run 和 `git diff --check`。尚未部署新增 Cloud Run Jobs，尚未跑真实 task fan-out smoke；正式替代 BQML 前仍需 sklearn parity passed 或 owner 接受新 Cloud Run baseline，并补 Python ledger vs SQL ledger 正式等价验收

...
- [x] OQ-010 / 数据治理：合并 `PRD_20260613_03` dividend 日常采集接入后完成 ingestion 镜像、window_refresh 部署与 dividend smoke
  说明：2026-06-13 已从 `main@3f017d5` 按 `orchestration/cloud_run_jobs/cloudbuild.ingestion.yaml` 重建 `ingestion:latest`，Cloud Build `9a6a778f-8942-49ac-93ec-9cb15b6596af`，digest `sha256:49bc7e1b59c88a78869238d3d3a8433b99fafb82a577f750eabcb797809ae493`。手工 smoke execution `ashare-ingest-current-scope-zzbfj` 对 `dividend_backfill` / `2026-06-12` 写入 dividend meta run `ing_dividend_backfill_20260612_20260612T173304Z`，`row_count=94`；`ashare_warehouse_window_refresh` 单独部署到 revision `000013-140`，`ashare_ods_ingestion_daily` / `ashare_pipeline_alert_checker` revision 保持 `000010-141` / `000004-f7b`，未动 full_rebuild opt-in、job spec、IAM 或 scheduler。

- [ ] OQ-010 / 数据治理：2026-06-15 scheduled run 后核验 `PRD_20260613_03` live 链路
  说明：核验清单：scheduled current_scope execution 使用 digest `sha256:49bc7e1b59c88a78869238d3d3a8433b99fafb82a577f750eabcb797809ae493` 或更新后明确批准的 digest；`ashare_meta.ingestion_run` / `ingestion_partition_status` 有 dividend 逐开市日 meta 行（`success` 或正常 `empty_return`）；`ashare_meta.v_ingestion_meta_missing` 为空；`daily_current` 事件两步 `windowed_weak_transform.dwd_stock_dividend_event` 与 `windowed_weak_qa.corporate_action_event_checks` task status 正常、无 `task_failure`；`ashare_dwd.dwd_stock_dividend_event` 可见上界推进到最新可见开市日。
...
- [x] OQ-010：owner 决定是否按 official ledger 复权漏损结果立修复 PRD
  说明：已闭环——owner 拍板立修复 PRD（`PRD_20260612_02`，三阶段实现并合并），DECISION-20260612-03 进一步 supersede 旧"未复权口径、持有期除权简化"约定并把研究 baseline 数字切到 CA-on 口径；量化漏损被 CA-on 记账吸收，无需另行永久披露。
...
- [x] OQ-010：按 `PRD_20260611_10` / `PRD_20260613_04` 完成自上而下整手组合构造 Phase 2 live
  说明：Phase 0 paper 与 PRD_20260613_01 P1 双选项前置判据均已完成；owner 通过 `PRD_20260613_04` 裁决 Phase 2 以 T0 / no-P1 口径进入。代码层修订已由 PR #215 合并，Phase 2 research-only continuous live 已完成：runner digest `sha256:1e91a5733df2cd7a38c2275cdb3a75f246fabe539c1e57db992c3a36bef5c9db`，execution `strategy1-backtest-report-job-j9m5t`，外接 QA 四件套全过，ADS/promotion 反查 0。报告 `docs/分析-topdownPhase2三方对比-20260613.md` 预登记判读为 topdown 证伪（长窗 CAGR `-77.13%`、Calmar `-0.772`、MaxDD `-99.95%`、平均现金 `76.31%`）；不得 promotion / accepted / default。

- [ ] OQ-010：topdown 证伪后选择现金拖累下一路线
  说明：PRD_20260613_04 Phase 2 T0 live 显示 topdown 真实 ledger 同时输给 v1 official baseline 与 Phase 0 paper T0，且平均现金仍高。后续若继续处理小资金 + 100 股整手现金拖累，应由 owner 选择是否转向碎股/资金口径、模型层 riskfeat、独立 overlay 或其他 alpha/组合路线；当前 topdown v2 T0 不应再作为 accepted / promotion / 默认构造候选。
...
- [x] OQ-010：完成 `PRD_20260613_02` 策略1 v3 Calmar 门合理性分析
  说明：分支 `codex/calmar-gate-analysis` 已新增只读脚本、报告和小 CSV，按 review 后硬要求覆盖窗口分层与反事实数据契约：八指数双窗口、3 年滚动 Calmar、当前 true5y CA-on baseline NAV exposure overlay 上界、A/B/C/D 反事实矩阵、portfolio 变体附表、全 gate 失败原因矩阵和满仓持指数误放行底线检查。关键事实：八指数长窗口 Calmar 最高 `0.1089`、短窗口最高 `1.3868`；当前 CA-on baseline 长窗口 Calmar `0.4103`、短窗口 Calmar `1.1041`；零摩擦 exposure 上界 Calmar `0.6455`。结论不是直接改门或 accepted，而是要求 owner 先裁决 acceptance 窗口口径，再决定 A/B/C/D 或分级展示；不改 contract / acceptance / registry，不写 BigQuery/GCS。

- [ ] OQ-010：基于 official / true-five-year / CA-on continuous 结果决定下一轮策略改进或 accepted baseline 路线
  说明：2021-2026 effective-window official continuous、true-five-year continuous 与 true-five-year CA-on continuous 都已成为 research evidence，但都尚未 accepted。Effective-window：compound CAGR=`0.12036528993503204`，MaxDD=`-0.4548151193656952`，legacy Sharpe=`0.6132671411257953`，v3 contract Sharpe=`0.5285475500566089`，contract Calmar=`0.26464663290635254`。True-five-year raw ledger：compound CAGR=`0.13852596798718442`，MaxDD=`-0.37189972934558946`，legacy Sharpe=`0.6834026126199905`，v3 contract Sharpe=`0.6075887294330015`，contract Calmar=`0.3724820349585642`。True-five-year CA-on 在 2026-06-12 dividend 补采后 resume 展示修正为 compound CAGR=`0.15357789449949522`，MaxDD=`-0.3742978588042647`，v3 contract Sharpe=`0.668539787795112`，contract Calmar=`0.41030930550903105`；仍未过 v3 hard gates（contract Sharpe `<0.70`、Calmar `<1.0`）。`PRD_20260613_02` 已补 Calmar 门合理性分析并确认结论依赖 acceptance 窗口口径：长窗口下 v3 Calmar `>1.0` 对当前路线明显偏高，短 replay 窗口又会放大 beta 误放行风险。baseline 口径已由 DECISION-20260612-03 裁决：研究 baseline 数字切为 CA-on、旧未复权约定 supersede、后续实验一律显式 `corporate_actions=cash_div_and_split_v1` / `dividend_tax_mode=flat_10pct`（代码默认不变；`DECISION-20260612-03` 文本不改）。剩余开放：owner 裁决 acceptance 窗口口径与 A/B/C/D 或分级展示，并继续围绕降低回撤、提升 risk-adjusted return、改进候选空间/风控/调仓参数做独立方案。不得把任一结果直接 promotion 或标 accepted。

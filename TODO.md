...
- [~] 按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 与 `docs/prd/PRD_20260605_02_策略1CloudRun轻量Task并发.md` 实现策略 1 Cloud Run 训练回测执行器：首版 Cloud Run runner 与 orchestrator 状态/锁增强已由 PR #56/#57 合并，真实 Cloud Run smoke 已跑通并通过 `16`/`17` QA（smoke 模式允许 parity 未通过）。2026-06-05 task fan-out P0 已完成代码实现、部署、低成本 smoke 和正式全量验收 run `s1_cloudrun_taskfanout_pvfq_n30_bw_h5_20260605_01` / `bt_s1_cloudrun_taskfanout_pvfq_n30_bw_h5_20260605_01`：`strategy1-prepare-matrix-job` 已从 `4 CPU / 16Gi` 提升到 `8 CPU / 32Gi`，全量训练面板 3,055,781 行的 `prepare_matrix` 成功；5 个 candidate task 全部成功，`cloudrun_select_register_predict`、`cloudrun_backtest_report`、内部 `10`/`12` QA、`16` smoke/evidence 模式和 `17` 均通过。但 `16` 正式 parity 在 `QA-CR-4` 失败，sklearn selected model valid RankIC `0.06665` 低于 BQML reference `0.09676`，`model_quality_status=model_quality_not_equivalent`。正式替代 BQML 前仍需 sklearn parity passed 或 owner 接受新 Cloud Run baseline，并补 Python ledger vs SQL ledger 正式等价验收
- [~] 按 `docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md` 实现 sklearn native 模型实验：PR #71 分支 `codex/implement-sklearn-native-search` 已新增 36 候选 manifest、candidate 参数训练、valid-only Top 5 排名、Top 5 独立预测/回测/报告/诊断 orchestrator 和 `sql/ml/strategy1/18_qa_sklearn_native_search_outputs.sql`；PR #71 review follow-up 已修复 native acceptance 缺失 top-minus-bottom hard gate、QA-SKN-13 NULL 漏洞、test RankIC 边界不一致、RankIC fallback 过宽、not_converged 未过滤、TopK 单候选失败中断全局 search，以及 runtime fetch SQL 缺少分区过滤的问题；已通过 Python `py_compile`、36-task plan dry-run、ranking fallback 小样例、runtime SQL dry-run、`18` BigQuery dry-run 和 `git diff --check`。尚未合并/部署镜像/实跑 36 候选；下一步是 review/merge 后部署 Cloud Run 镜像并跑真实 search，最终决定是否接受 `cloud_run_sklearn_native_baseline_v1`
...
- [~] OQ-005 GCP 数据流水线落地：`docs/prd/PRD_20260603_03_GCP数据流水线方案.md` 与 `docs/prd/PRD_20260605_01_OQ005剩余调度链路.md` 已定义 Cloud Run Jobs 采集、Dataform / BigQuery Studio pipeline 做 ODS→ADS、Cloud Composer 编排；Phase 0 已实现采集 manifest、14 张 schema contract、meta 表 DDL 与采集脚本 stub；Phase 1/1.5/1.6 已实现 endpoint worker、GCS staging/publish、小范围写入 smoke、ODS 可读性 QA、Composer 3 环境和每日轻量 readiness DAG；Phase 1.7 已新增 `ashare-ingest-current-scope` 单 execution 生产入口、Direct VPC egress + Cloud NAT 固定出口，修复 Composer scheduler queued 问题为 default Celery queue 路径，并完成 `2026-05-20` 至 `2026-06-04` 当前范围 ODS 生产采集与 readiness 验收。Phase 2.0 BigQuery SQL 兼容路径已由 PR #61 进入 `main`。Phase 2.2 股票 DWD/DWS 窗口刷新已进入 `main` 并部署到 Composer：`daily_current` 默认刷新最近 20 个交易日，`backfill` 保持显式日期；已完成 `2026-05-08..2026-06-04` 20 个交易日估值缺口回填、backfill / qa_only / daily_current smoke 和状态表验收，ODS `daily_basic` → DWD valuation → DWS valuation 20 日窗口键数均为 110,012，缺失为 0。本次 hardening 补齐 `daily_current` 非交易日 `date_to` / `business_date` 归一到最近 SSE 开市日，避免周末/节假日窗口退化；PR #70 review follow-up 已将估值覆盖 QA 的 backfill 范围收敛到实际写入窗口，并让 QA-WIN-18 按 price-driven feature universe 与 `total_mv_cny/circ_mv_cny` 口径检查 `has_valuation_data`。下一步进入 Dataform definitions、策略 runner/report 可选分支、告警、补跑和生产运维观测闭环

...
- [ ] 按 `docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md` 实现 sklearn native 模型实验：固定 `pv_fin_quality + 30/5% + biweekly + 5d` 交易口径，一次 `prepare_matrix` 后并发训练 36 个 LogisticRegression 候选，只按 valid 指标选 Top 5，再对 Top 5 跑完整预测/组合/回测/报告/诊断和 `18` QA，决定是否接受 `cloud_run_sklearn_native_baseline_v1`
...
- [x] 新增策略 1 scikit-learn native 模型实验 PRD：`docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md`，定义 Cloud Run sklearn parity 未通过后不再继续逆向贴近 BQML，而是固定当前交易参数、并发训练 36 个 sklearn 原生 LogisticRegression 候选、valid-only 选 Top 5 回测，并以独立 native acceptance gate 决定是否建立新的 Cloud Run sklearn baseline；本次只写文档，未实现代码、未部署 Cloud Run Job、未执行 BigQuery

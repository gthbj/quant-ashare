...
- [ ] OQ-010：按 `PRD_20260611_03` 实现 synthetic continuous merge 与正式 continuous ledger
  说明：PR #167 已合并代码侧能力：`quant_ashare.strategy1.synthetic_continuous` 按 manifest 合并逐年 refit prediction slice、写 synthetic selected registry 和统一 synthetic model/run；`qa_continuous_backtest_outputs` 已登记 catalog，覆盖 manifest/hash/source lineage、source/target 行数、valid 排除、交易日历覆盖和 continuous ledger 不变式。Official merge 首次执行暴露 source prediction join 缺少静态 `predict_date` partition filter，BigQuery job `cde0ff0e-fe0a-4124-89d1-c5406a8c5caa` 失败；当前分支 `codex/synthetic-continuous-partition-filter` 已补 synthetic insert 与 QA source count 的静态窗口过滤，待合并后用 `--force-replace` 重跑 official synthetic merge、跑单一 continuous backtest（`--skip-diagnosis --skip-tail-risk --skip-qa`），再执行 `qa_continuous_backtest_outputs` 与 `qa_lot_aware_ledger_outputs`。

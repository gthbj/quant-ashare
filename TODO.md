...
- [~] 修复 ODS 外部表 Parquet schema mismatch：PR 实现已提交（schema contract YAML × 10 endpoint + 修复/验证脚本 + QA SQL + README），P0 `stk_limit` 待在 BigQuery 实际执行修复并验证，再分批修其余 9 张 P1/P2/P3 表；默认从 GCS 原 Parquet 按 schema contract 重写，不从 API 重拉覆盖历史 raw

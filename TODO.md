...
- [~] 按 `docs/prd/PRD_20260605_03_策略1Sklearn模型实验.md` 实现 sklearn native 模型实验：分支 `codex/implement-sklearn-native-search` 已新增 36 候选 manifest、candidate 参数训练、valid-only Top 5 排名、Top 5 独立预测/回测/报告/诊断 orchestrator 和 `sql/ml/strategy1/18_qa_sklearn_native_search_outputs.sql`；已通过 Python `py_compile`、36-task plan dry-run、配置校验、`18` BigQuery dry-run 和 `git diff --check`。尚未合并/部署镜像/实跑 36 候选；下一步是 review/merge 后部署 Cloud Run 镜像并跑真实 search，最终决定是否接受 `cloud_run_sklearn_native_baseline_v1`

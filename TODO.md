...
- [ ] OQ-010：按 `PRD_20260611_03` 实现 synthetic continuous merge 与正式 continuous ledger
  说明：PR #167/#168 已合并 synthetic continuous 能力与 partition-filter hotfix。Official synthetic merge 已用 `--force-replace` 成功写入 registry rows=`1` / prediction rows=`2643406`（insert job `f8953afd-b95b-4b20-82f7-2af153bed998`）；single continuous ledger 已成功（Cloud Run execution `strategy1-backtest-report-job-w5k24`）；`qa_lot_aware_ledger_outputs` 已通过（job `4dcfd716-6cea-4efd-b1f9-d7f195d1f004`）。剩余 blocker：`qa_continuous_backtest_outputs` 的 `QA-CONT-6` 发现 resolved manifest 把 refit train window 误作 selection valid window；当前分支 `codex/synthetic-continuous-valid-windows` 已改为从 refit row 的 `model_params_json.source_run_id` 指向的原 selection registry 读取 valid window，待合并后重跑 synthetic merge 与 `qa_continuous_backtest_outputs`。

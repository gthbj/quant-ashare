...
- [x] OQ-010 / 工程治理：实现 `PRD_20260612_05` Strategy1 包结构 Phase E 收尾 Batch 2
  说明：分支 `codex/prd05-batch2` 已完成 Batch 2：`state.py` / `task_fanout.py` 迁入 `src/quant_ashare/strategy1/`，scripts 同名路径改为兼容 shim；src 内对 `state` / `task_fanout` 的反向 import 清零。`annual_pipeline_scheduler.py` 复用迁入后的 state helpers，`GcloudExecutionClient.describe` 恢复失败 warning；三类锁语义只补 docstring 出处注记，未合并 reclaim / heartbeat 行为。新增 fake GCS lease 单测与 Batch 2 兼容符号快照；本轮未改训练/回测/组合语义，未触碰 Cloud Run job spec/args/镜像/IAM，未写 BigQuery/GCS。Batch 3 仍为后续独立 PR。

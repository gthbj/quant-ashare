...
- [~] 按 `docs/prd/PRD_20260604_04_策略1CloudRun训练回测.md` 实现策略 1 Cloud Run 训练回测执行器：首版 Cloud Run runner 已由 PR #56 合并，包含 sklearn logistic 训练/预测、Python fresh-start `ledger_exec_v1`、Cloud Run orchestrator、Docker/Cloud Build 配置、运行手册、meta 扩展 SQL 和 `16_qa_cloudrun_runner_outputs.sql`；当前分支 `codex/cloudrun-orchestrator-state-lock` 补 orchestrator 状态表写入、GCS generation-guarded lock、`--resume` / `--resume-from-step` 和 `17_qa_cloudrun_orchestrator_status.sql`，PR #57 review follow-up 已补 stale lock 回收前检查 Cloud Run execution terminal、失锁 cancel execution、heartbeat 异常兜底和 execution id QA，已通过本地 dry-run / py_compile / BigQuery dry-run / `git diff --check`；待 PR review/merge 后做真实 Cloud Run/BQ smoke、sklearn vs BQML parity 验收、Python ledger vs SQL ledger 等价验收
...
- [x] 新增 OQ-005 剩余 ODS→ADS 生产调度链路 PRD：`docs/prd/PRD_20260605_01_OQ005剩余调度链路.md`，聚焦当前生产采集入口之后的 ODS gate、ODS→DIM/DWD/DWS/ADS 转换、ADS 契约隔离、metadata、QA、pipeline 状态、告警、补跑、Dataform/BigQuery SQL 双路径和 OQ-005 关闭标准；本次只写文档，未实现代码、未部署任务、未执行 BigQuery

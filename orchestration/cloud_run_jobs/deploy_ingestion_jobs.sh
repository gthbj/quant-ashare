...
deploy_job() {
  ...
  gcloud run jobs deploy "${job_name}" \
    ...
    --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},BQ_LOCATION=${REGION},INGESTION_MANIFEST=${INGESTION_MANIFEST},TUSHARE_HTTP_URL=${TUSHARE_HTTP_URL},TUSHARE_ROW_LIMIT=${TUSHARE_ROW_LIMIT},TUSHARE_THROTTLE_SECONDS=${TUSHARE_THROTTLE_SECONDS}" \

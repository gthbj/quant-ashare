# orchestration - A 股 GCP 调度流水线

> 文档维护：GPT-5（最近更新 2026-06-05）

本目录放置 GCP 生产流水线的部署和调度配置：

- `cloud_run_jobs/`：Tushare 或 Tushare 兼容 API 到 GCS Parquet 的 Cloud Run Jobs 镜像、部署脚本和作业模板。
- `composer/`：Cloud Composer DAG，用于串联每日采集、ODS 可读性检查、Dataform/BigQuery 转换和 QA。

采集 worker 代码仍位于 `scripts/ingestion/`，ODS 到 DIM/DWD/DWS/ADS 的 SQL/建模资产仍位于现有 `sql/`、`configs/`、`docs/` 目录。本目录只承载调度和部署入口，不保存 token、service account key 或任何凭据。

当前调度体系已提供可部署 jobs、真实 worker、每日窗口刷新和 Composer DAG 的 BigQuery SQL 兼容调度闭环：

- Cloud Run Jobs 使用 `TUSHARE_TOKEN` Secret Manager 注入方式；每日生产入口使用 `ashare-ingest-current-scope` 单 execution 顺序执行当前 14 个 ODS endpoint；Jobs 通过 Direct VPC egress + Cloud NAT + 区域静态 IP 固定出口。
- Tushare 官方或兼容 API 地址通过 `TUSHARE_HTTP_URL` 环境变量注入。
- Jobs 模板默认追加 `--dry-run`，不会请求 API，也不会写 GCS；Composer 生产路径在 `ashare_pipeline_dry_run=false` 时显式传入 `--allow-gcs-write`，手工 DAG run 可用 `pipeline_dry_run` / `dry_run` 单次覆盖。
- 本地可用 `--skip-gcs-write` 做只读 API smoke，验证 API、分页和 schema cast。
- worker 真实写入要求显式传 `--allow-gcs-write`，防止误写生产 GCS。
- 已用小范围 `index_daily_000852_SH / 20260603` 做 GCS 写入 smoke，并通过 ODS 外部表读取验证。
- Composer DAG 已接入生产每日采集、`sql/qa/09_ods_daily_partition_readiness.sql` 每日 ODS readiness、DIM 小表刷新、P0 metadata 恢复、股票 DWD/DWS 窗口刷新、`sql/qa/10_windowed_stock_refresh_checks.sql` 窗口 QA、`pipeline_run` / `pipeline_task_status` 状态回写、`skip_ingestion` smoke、`qa_only` 只读验收、ADS 契约手工初始化和 BigQuery SQL 兼容转换分支。
- 当前 Airflow 变量为 `ashare_pipeline_dry_run=false`、`ashare_warehouse_mode=daily_current`、`ashare_transform_backend=bq_sql`、`ashare_enable_ads_contract_init=false`、`ashare_enable_full_refresh=false`。
- 每日默认主链只读业务日分区或窗口影响范围；2019+ 全历史 schema 检查、DWD/DWS/metadata/QA 全量刷新通过 `warehouse_mode=full_rebuild` 或 `warehouse_mode=full_rebuild_compat` 手工启用。兼容变量 `ashare_enable_full_refresh=true` 会记录为 `full_rebuild_compat`。
- `sql/qa/06_ods_parquet_schema_checks.sql` 和 `sql/qa/08_ods_external_readability_checks.sql` 是维护/全量刷新检查，不作为每日调度默认门禁。

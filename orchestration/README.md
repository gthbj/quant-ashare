...
当前 Phase 2.0 已提供可部署 jobs、真实 worker 和 Composer DAG 的 BigQuery SQL 兼容调度闭环：
...
- Composer DAG 已接入生产每日采集、`sql/qa/09_ods_daily_partition_readiness.sql` 每日 ODS readiness、`pipeline_run` / `pipeline_task_status` 状态回写、`skip_ingestion` smoke、`qa_only` 只读验收、ADS 契约手工初始化和 BigQuery SQL 兼容转换分支。
...
- 每日默认主链只读业务日分区或近期小窗口；2019+ 全历史 schema 检查、DIM/DWD/DWS/metadata/QA 全量刷新通过 `warehouse_mode=full_rebuild` 或 `warehouse_mode=full_rebuild_compat` 手工启用。兼容变量 `ashare_enable_full_refresh=true` 会记录为 `full_rebuild_compat`。
